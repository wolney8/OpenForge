...
def test_mbb_extra_place_reference_example() -> None:
    ...
    assert (
        result.standard_place_bookie_pnl + result.standard_place_exchange_pnl
        == result.standard_place_pnl
    )
...
def test_extra_place_settlement_and_void() -> None:
    ...
    assert calculate_each_way_extra_place(
        EachWayCalculationInput(**payload, result="Extra Place")
    ).final_value == Decimal("30.53")
    assert calculate_each_way_extra_place(
        EachWayCalculationInput(**payload, result="Void/NR")
...
def test_each_way_current_value_excludes_extra_place_only_branch() -> None:
    ...
    assert result.current_value == min(
        result.first_place_pnl, result.standard_place_pnl, result.unplaced_pnl
    )
...
def test_historical_actual_legs_remain_authoritative() -> None:
    result = calculate_each_way_extra_place(
        EachWayCalculationInput(
            ...
            place_term_denominator="5",
            win_lay_odds="29",
            place_lay_odds="7.962",
            actual_win_lay_stake="4.48",
            actual_place_lay_stake="3.75",
            result="Extra Place",
        )
    )
