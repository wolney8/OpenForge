...
def test_default_database_path_targets_private_storage() -> None:
    assert Settings(_env_file=None).database_path == (
        SOURCE_ROOT / "data/private/db/openforge.sqlite3"
    )


def test_default_backup_path_targets_private_storage() -> None:
    assert Settings(_env_file=None).backup_path == SOURCE_ROOT / "data/private/backups"


def test_default_master_account_catalogue_is_a_committed_reference_source() -> None:
    assert Settings(_env_file=None).account_catalogue_source_path == (
        SOURCE_ROOT / "data/reference/master-account-catalogue.json"
    )
