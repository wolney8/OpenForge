...
export function CoreLayPlanner({ accounts, basis, defaultCommission, exchangeCommissions, actualLiability, form, inspectionId,
  ...
  return <div className="calculator-shell" data-pd-id={inspectionId}>
      ...
      <div className="form-grid calculator-paired-segments calculator-paired-rows-2" data-pd-id={`${inspectionId}.paired-segments`}>
        ...
        <section className="calculator-segment calculator-segment-lay calculator-paired-segment" style={{"--calculator-segment-field-rows":2,"--calculator-segment-span":4} as CSSProperties}>
          <CalculatorSegmentEyebrow>Lay bet</CalculatorSegmentEyebrow><div className="calculator-segment-grid calculator-paired-segment-fields">
            ...
            <label className="field-control"><span>Commission (%)</span><CommissionInput aria-label="Planning Commission (%)" onRatioChange={value => { if (value === commission && origin === "override") return; invalidate(); setCommission(value); if (!hasActual) setActualCommission(value); setOrigin("override"); }} readOnly={readOnly} value={commission} /></label>
          </div></section>
      </div>
    ...
    <div className="calculator-band calculator-band-secondary" aria-busy={busy}>
      {mode === "Advanced" ? <section className="stack" data-pd-id={`${inspectionId}.advanced`}>
        {(["Underlay","Overlay"] as const).map(name => { const r = references.find(r => r.strategy === name); return <CalculatorReferenceSection key={name}
          ...
          action={<button aria-pressed={strategy === name} className="button-link" disabled={disabled || !r} onClick={() => selectStrategy(name)} type="button">Apply {name}</button>} />; })}
        <CalculatorReferenceSection busy={disabled} description="Editable planned lay; actual placement is confirmed separately."
          inspectionId={`${inspectionId}.custom`} title="Custom Lay reference" rows={custom ? rowsFor(custom) : []}
        ...
        {Number.isFinite(minimum) && Number.isFinite(maximum) && maximum > minimum ? <SingleLayCustomSlider
          ...
          onDraft={editCustom} onCommit={editCustom} onMinimumChange={setMinimumText} onMaximumChange={setMaximumText} /> : <p className="field-hint">Enter valid minimum and maximum bounds to use the slider.</p>}
      </section> : null}
      <CalculatorReferenceSection busy={disabled} description="Reviewed target hedge, separate from actual recorded placement."
        ...
        action={mode === "Advanced" ? <button className="button-link" disabled={disabled || !standard} onClick={() => selectStrategy("Standard")} type="button">Apply Standard</button> : undefined} />
