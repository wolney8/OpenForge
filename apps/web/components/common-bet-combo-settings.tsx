...
const emptyDraft: CommonBetCombo = {
  ...
  quick_add: { enabled: false, display_label: "", supported_ledgers: [], enabled_fields: [], defaults: {} },
  status: "Active",
  version: 0,
  sort_order: 0,
};
  ...
  return (
    <>
      <section className="content-panel stack" data-pd-id="common-bet-combos.section">
        ...
        <h2>Common Bet Combos And Quick Add Loadouts</h2>
        <button
              ...
              {draft ? (
                <div className="stack common-bet-combo-editor" data-pd-id="common-bet-combos.editor">
                  <div className="form-grid common-bet-combo-form-grid">
                    ...
                    <label className="profile-filter-chip common-bet-combo-loadout-toggle"><input checked={draft.quick_add.enabled} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, enabled: event.target.checked, supported_ledgers: event.target.checked && !draft.quick_add.supported_ledgers.length ? [draft.ledger_type] : draft.quick_add.supported_ledgers } })} type="checkbox" /><span>Available In Quick Add</span></label>
                    {draft.quick_add.enabled ? <><label className="field-control"><span>Quick Add Label</span><input maxLength={80} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, display_label: event.target.value } })} placeholder={draft.name} value={draft.quick_add.display_label} /></label><fieldset className="common-bet-combo-choice-group"><legend>Quick Add Ledgers</legend>{(["Sportsbook", "Free Bets", "Casino", "Cash Adjustments", "Extra Place"] as const).map((ledger) => <label className="profile-filter-chip" key={ledger}><input checked={draft.quick_add.supported_ledgers.includes(ledger)} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, supported_ledgers: event.target.checked ? [...new Set([...draft.quick_add.supported_ledgers, ledger])] : draft.quick_add.supported_ledgers.filter((value) => value !== ledger) } })} type="checkbox" /><span>{ledger}</span></label>)}</fieldset></> : null}
