...
export type CommonBetCombo = {
  preset_id: string;
  name: string;
  ledger_type: "Sportsbook" | "Casino" | "Extra Place";
  bookmaker: string;
  bookmakers: string[];
  offer_type: string;
  bet_type: string;
...
const quickActionFieldsByLedger = {
  Sportsbook: ["offerName", "bookmaker", "betType", "offerType", "fixtureType", "event", "market", "stake", "backOdds", "exchange", "layMode"],
  "Free Bets": ["freeBetValue", "retention", "bookmaker", "fixtureType", "betType", "exchange", "offerName"],
  Casino: ["offerName", "offerType", "bookmaker", "game", "spinCount", "spinStake", "reward", "convertedWin"],
  "Cash Adjustments": ["adjustmentType", "linkedAccount", "amount", "direction", "adjustmentDate", "notes"],
  "Extra Place": ["runner", "race", "bookmaker", "eachWayStake", "placeTermDenominator", "bookmakerPlaces", "exchangePlaces", "winExchange", "placeExchange", "winLayOdds", "placeLayOdds"],
} as const;
const quickActionFieldLabel = (field: string) => field.replace(/([A-Z])/g, " $1").replace(/^./, (value) => value.toUpperCase());
const emptyDraft: CommonBetCombo = {
  ...
  quick_add: { enabled: false, display_label: "", supported_ledgers: [], enabled_fields: [], defaults: {}, enforcement: "optional", allowed_profile_override_fields: [] },
  status: "Active",
  version: 0,
  sort_order: 0,
};
  ...
  return (
    <>
      <section className="content-panel stack" data-pd-id="common-bet-combos.section">
        ...
        <h2>Quick Actions</h2>
        <section className="stat-strip settings-stat-strip" aria-label="Quick Action summary">
          <article className="stat-card"><span>Active Actions</span><strong>{rows.filter((row) => row.status === "Active").length}</strong><small>Available templates</small></article>
          <article className="stat-card"><span>Required</span><strong>{rows.filter((row) => row.status === "Active" && row.quick_add.enabled && row.quick_add.enforcement === "required").length}</strong><small>Forced for eligible profiles</small></article>
              ...
              {draft ? (
                <div className="stack common-bet-combo-editor" data-pd-id="common-bet-combos.editor">
                  <div className="form-grid common-bet-combo-form-grid">
                    ...
                    <label className="field-control"><span>Ledger</span><select aria-label="Combo ledger" data-pd-id="common-bet-combos.editor.ledger" onChange={(event) => setDraft({ ...draft, ledger_type: event.target.value as CommonBetCombo["ledger_type"], offer_type: "" })} value={draft.ledger_type}><option>Sportsbook</option><option>Casino</option><option>Extra Place</option></select></label>
                    <label className="field-control"><span>Offer Type</span><select onChange={(event) => setDraft({ ...draft, offer_type: event.target.value })} value={draft.offer_type}><option value="">Select</option>{(draft.ledger_type === "Casino" ? casinoOfferTypeOptions : draft.ledger_type === "Extra Place" ? ["Each Way", "Extra Place"] : sportsbookOfferTypeOptions).map((value) => <option key={value}>{value}</option>)}</select></label>
                    ...
                    {draft.ledger_type === "Sportsbook" || draft.ledger_type === "Extra Place" ? <><label className="field-control"><span>Fixture Type</span><select onChange={(event) => setDraft({ ...draft, fixture_type: event.target.value })} value={draft.fixture_type}><option value="">Not set</option>{fixtureTypeOptions.map((value) => <option key={value}>{value}</option>)}</select></label><label className="field-control"><span>Default Back Stake</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, default_back_stake: event.target.value })} value={draft.default_back_stake} /></label><label className="field-control"><span>Minimum Back Odds</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, minimum_back_odds: event.target.value })} value={draft.minimum_back_odds} /></label>{draft.ledger_type === "Sportsbook" ? <label className="field-control"><span>Preferred Strategy</span><select aria-describedby="preferred-strategy-help" onChange={(event) => setDraft({ ...draft, default_strategy: event.target.value })} value={draft.default_strategy || ""}>{strategies.map((value) => <option key={value || "none"} value={value}>{value ? value === "Custom" ? "Custom Lay" : value === "Multilay" ? "Multi Lay" : value : "No preference"}</option>)}</select><small className="field-support-text" id="preferred-strategy-help">Sets the initial strategy only. It never prevents a later change.</small></label> : null}</> : <><label className="field-control"><span>Game / Slot</span><input maxLength={200} onChange={(event) => setDraft({ ...draft, game: event.target.value })} value={draft.game} /></label><label className="field-control"><span>Cash Stake</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, cash_stake: event.target.value })} value={draft.cash_stake} /></label><label className="field-control"><span>Credit Amount</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, credit_amount: event.target.value })} value={draft.credit_amount} /></label><label className="field-control"><span>Bonus Amount</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, bonus_amount: event.target.value })} value={draft.bonus_amount} /></label><label className="field-control"><span>Wager Multiplier</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, wager_multiplier: event.target.value })} value={draft.wager_multiplier} /></label><label className="field-control"><span>Required Spins</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, required_spins: event.target.value })} value={draft.required_spins} /></label><label className="field-control"><span>Spin Stake</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, spin_stake: event.target.value })} value={draft.spin_stake} /></label><label className="field-control"><span>Free Spins Awarded</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, free_spins_awarded: event.target.value })} value={draft.free_spins_awarded} /></label><label className="field-control"><span>Free Spins Value</span><input inputMode="decimal" onChange={(event) => setDraft({ ...draft, free_spins_value: event.target.value })} value={draft.free_spins_value} /></label></>}
                    <label className="profile-filter-chip common-bet-combo-loadout-toggle"><input checked={draft.quick_add.enabled} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, enabled: event.target.checked, enforcement: event.target.checked ? draft.quick_add.enforcement : "optional", supported_ledgers: event.target.checked && !draft.quick_add.supported_ledgers.length ? [draft.ledger_type] : draft.quick_add.supported_ledgers } })} type="checkbox" /><span>Available In Quick Actions</span></label>
                    {draft.quick_add.enabled ? <><label className="field-control"><span>Quick Action Label</span><input maxLength={80} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, display_label: event.target.value } })} placeholder={draft.name} value={draft.quick_add.display_label} /></label><fieldset className="common-bet-combo-choice-group"><legend>Quick Action Ledgers</legend>{(["Sportsbook", "Free Bets", "Casino", "Cash Adjustments", "Extra Place"] as const).map((ledger) => <label className="profile-filter-chip" key={ledger}><input checked={draft.quick_add.supported_ledgers.includes(ledger)} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, supported_ledgers: event.target.checked ? [...new Set([...draft.quick_add.supported_ledgers, ledger])] : draft.quick_add.supported_ledgers.filter((value) => value !== ledger), enabled_fields: event.target.checked ? draft.quick_add.enabled_fields : draft.quick_add.enabled_fields.filter((field) => !quickActionFieldsByLedger[ledger].includes(field as never)), allowed_profile_override_fields: event.target.checked ? draft.quick_add.allowed_profile_override_fields : draft.quick_add.allowed_profile_override_fields.filter((field) => !quickActionFieldsByLedger[ledger].includes(field as never)) } })} type="checkbox" /><span>{ledger}</span></label>)}</fieldset><fieldset className="common-bet-combo-choice-group"><legend>Quick Action Fields</legend>{draft.quick_add.supported_ledgers.flatMap((ledger) => quickActionFieldsByLedger[ledger]).filter((field, index, values) => values.indexOf(field) === index).map((field) => <label className="profile-filter-chip" key={field}><input checked={draft.quick_add.enabled_fields.includes(field)} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, enabled_fields: event.target.checked ? [...new Set([...draft.quick_add.enabled_fields, field])] : draft.quick_add.enabled_fields.filter((value) => value !== field), allowed_profile_override_fields: event.target.checked ? draft.quick_add.allowed_profile_override_fields : draft.quick_add.allowed_profile_override_fields.filter((value) => value !== field) } })} type="checkbox" /><span>{quickActionFieldLabel(field)}</span></label>)}</fieldset><fieldset className="common-bet-combo-choice-group"><legend>Profile Override Fields</legend>{draft.quick_add.enabled_fields.map((field) => <label className="profile-filter-chip" key={field}><input checked={draft.quick_add.allowed_profile_override_fields.includes(field)} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, allowed_profile_override_fields: event.target.checked ? [...new Set([...draft.quick_add.allowed_profile_override_fields, field])] : draft.quick_add.allowed_profile_override_fields.filter((value) => value !== field) } })} type="checkbox" /><span>{quickActionFieldLabel(field)}</span></label>)}</fieldset><label className="profile-filter-chip"><input checked={draft.quick_add.enforcement === "required"} onChange={(event) => setDraft({ ...draft, quick_add: { ...draft.quick_add, enforcement: event.target.checked ? "required" : "optional" } })} type="checkbox" /><span>Required For Eligible Profiles</span></label></> : null}
