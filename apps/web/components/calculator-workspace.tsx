  ...
  return <section aria-labelledby="calculator-workspace-title" className={`content-panel stack sportsbook-page-shell${popout ? " calculator-popout-workspace" : ""}`} data-pd-id="calculators.workspace">
    ...
    {family === "matched-betting" ? <div className="calculator-panel-shell"><div className="calculator-shell">
      ...
      {result ? <div className="calculator-band calculator-band-secondary" data-pd-id="calculators.matched-betting.results">
        {showAdvancedReferences ? <section className="calculator-advanced-reference-group" data-pd-id={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.advanced" : "calculators.matched-betting.advanced"}>{inputs.betType === "bonus_lock_in" && referenceStandard ? <CalculatorReferenceSection busy={isCalculating || invalid} description="Equalised reference after the selected bonus assumption and penny placement." inspectionId="calculators.bonus-lock-in.standard" rows={[{ label: "Lay stake", value: referenceStandard.lay_stake, copyable: true }, { label: "Liability", value: referenceStandard.liability }, { label: "Back wins", value: referenceStandard.back_wins_total }, { label: "Back loses", value: referenceStandard.back_loses_total }]} title="Standard reference" /> : null}<div className="calculator-advanced-reference-pair">{referenceUnderlay ? <CalculatorReferenceSection busy={isCalculating || invalid} action={<button className="button-link" disabled={isCalculating || invalid} onClick={() => updatePatch({ strategy: "Underlay", manualLayStake: "" })} type="button">Apply Underlay</button>} description="Lower lay reference; penny placement can leave a small residual." inspectionId={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.underlay" : "calculators.matched-betting.underlay"} rows={[{ label: "Lay stake", value: referenceUnderlay.lay_stake, copyable: true }, { label: "Liability", value: referenceUnderlay.liability }, { label: "Back wins", value: referenceUnderlay.back_wins_total }, { label: "Back loses", value: referenceUnderlay.back_loses_total }]} title="Underlay reference" tone="underlay" /> : <CalculatorReferenceSection busy={isCalculating || invalid} description="No valid non-negative endpoint exists for these inputs." inspectionId="calculators.matched-betting.underlay-unavailable" rows={[]} title="Underlay unavailable" tone="underlay" />}{referenceOverlay ? <CalculatorReferenceSection busy={isCalculating || invalid} action={<button className="button-link" disabled={isCalculating || invalid} onClick={() => updatePatch({ strategy: "Overlay", manualLayStake: "" })} type="button">Apply Overlay</button>} description="Upper lay reference; penny placement can leave a small residual." inspectionId={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.overlay" : "calculators.matched-betting.overlay"} rows={[{ label: "Lay stake", value: referenceOverlay.lay_stake, copyable: true }, { label: "Liability", value: referenceOverlay.liability }, { label: "Back wins", value: referenceOverlay.back_wins_total }, { label: "Back loses", value: referenceOverlay.back_loses_total }]} title="Overlay reference" tone="overlay" /> : <CalculatorReferenceSection busy={isCalculating || invalid} description="No valid non-negative endpoint exists for these inputs." inspectionId="calculators.matched-betting.overlay-unavailable" rows={[]} title="Overlay unavailable" tone="overlay" />}</div><section className="calculator-custom-reference-group" data-pd-id="calculators.matched-betting.custom-group">{referenceCustom ? <CalculatorReferenceSection busy={isCalculating || invalid} description="Editable planned reference, not evidence of an actual matched lay." inspectionId="calculators.matched-betting.custom" rows={[{ label: "Lay stake", value: referenceCustom.lay_stake, copyable: true }, { label: "Liability", value: referenceCustom.liability }, { label: "Back wins", value: referenceCustom.back_wins_total }, { label: "Back loses", value: referenceCustom.back_loses_total }]} title="Custom Lay reference" tone="custom" action={<Field error={inputs.customLayDraft && !isPositiveAmount(inputs.customLayDraft) ? "Enter a positive Custom lay stake." : null} id="custom-reference-lay" label="Custom Lay" onChange={(value) => updatePatch({ strategy: "Custom", manualLayStake: value, customLayDraft: value })} value={inputs.customLayDraft || referenceCustom.lay_stake} />} /> : null}<SingleLayCustomSlider current={advancedCurrent} maximum={advancedMaximum} maximumText={advancedMaximumText} minimum={advancedMinimum} minimumText={advancedMinimumText} onCommit={(value) => updateBonusCustomStake(Number(value))} onDraft={(value) => updateBonusCustomStake(Number(value))} onMaximumChange={(value) => update("customMaximum", value)} onMinimumChange={(value) => update("customMinimum", value)} /></section></section> : <div className="calculator-panel-card calculator-result-panel"><FinancialValueReplayGroup><article className="calculator-result-card"><div className="calculator-result-card-heading"><strong>Standard reference</strong></div><dl className="calculator-result-card-values"><ResultValue copyable disabled={isCalculating || invalid} dataPdId="calculators.matched-betting.copy-lay-stake" label="Lay stake required" value={result.selected_lay_stake} /><ResultValue label="Liability" value={result.liability} /><ResultValue label={inputs.betType === "cashback" ? "Without cashback (qualifying result)" : "Matched result"} value={result.matched_result} />{inputs.betType === "profit_boost" ? <ResultValue label="Effective boosted odds" value={result.effective_back_odds} money={false} /> : null}</dl></article></FinancialValueReplayGroup></div>}<CalculatorOutcomes busy={isCalculating || invalid} columns={["Bookmaker", "Exchange", "Bonus / cashback"]} inspectionId="calculators.outcomes" rows={result.outcomes.map((outcome, index) => ({ key: outcome.key, label: outcome.label, copyableTotal: true, tone: index === 0 ? "positive" : "exchange", components: [[outcome.bookmaker_component], [outcome.exchange_component], [outcome.promotion_component]], total: outcome.total }))} summary={<span>{inputs.betType === "cashback" ? "Without cashback " : "Matched result "}<CalculatorOutcomeValueDisplay label={inputs.betType === "cashback" ? "Without cashback" : "Matched result"} value={result.matched_result} /></span>} /></div> : null}
...
export function MultiLayCalculator({ exchanges, onState, search, planning = false }: { exchanges: ExchangeOption[]; onState: (params: URLSearchParams) => void; search: URLSearchParams; planning?: boolean }) {
  ...
  return <div className="calculator-panel-shell" data-pd-id="calculators.multi-lay.presentation"><div className="calculator-shell">
    <div className="calculator-band calculator-band-primary"><div className="calculator-segment calculator-segment-back"><CalculatorSegmentEyebrow>Back bet</CalculatorSegmentEyebrow><div className="calculator-segment-grid calculator-segment-grid-back">
      <SelectField disabled={planning} id="multi-backing-type" label="Back type" value={inputs.backingType} onChange={(value) => update({ ...inputs, backingType: value as MultiLayInputs["backingType"] })} options={[["normal", "Normal"], ["free_bet_snr", "Free Bet (SNR)"], ["money_back", "Money Back if bet loses"]]} />
    ...
    <div className="calculator-band calculator-band-primary calculator-band-single calculator-band-multilay">
      <div className="calculator-panel-card calculator-panel-card-multilay">
        ...
        <div className="stack">
          ...
          {!planning ? <div className="calculator-reference-action"><Field error={multiRangeError(inputs.customMultiplier, 0, Infinity, "Enter zero or a positive multiplier.")} id="multi-custom-multiplier" label="Custom multiplier" onChange={(value) => update({ ...inputs, strategy: "custom", customMultiplier: value })} value={inputs.customMultiplier} /></div> : null}
          {result && !planning ? <SingleLayCustomSlider ariaLabel="Custom Multi-Lay multiplier" centre={1} current={current} maximum={maximum} maximumText={inputs.customMaximum || result.default_maximum_multiplier} minimum={minimum} minimumText={inputs.customMinimum || result.default_minimum_multiplier} onCommit={(value) => updateLive({ ...inputs, strategy: "custom", customMultiplier: value })} onDraft={(value) => updateLive({ ...inputs, strategy: "custom", customMultiplier: value })} onMaximumChange={(value) => update({ ...inputs, customMaximum: value })} onMinimumChange={(value) => update({ ...inputs, customMinimum: value })} unit="multiplier" /> : null}
          </section> : null}
