  ...
  return <section aria-labelledby="calculator-workspace-title" className={`content-panel stack sportsbook-page-shell${popout ? " calculator-popout-workspace" : ""}`} data-pd-id="calculators.workspace">
    ...
    {family === "matched-betting" ? <div className="calculator-panel-shell"><div className="calculator-shell">
      ...
      {result ? <div className="calculator-band calculator-band-secondary" data-pd-id="calculators.matched-betting.results">
        {showAdvancedReferences ? <section className="calculator-advanced-reference-group" data-pd-id={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.advanced" : "calculators.matched-betting.advanced"}>{inputs.betType === "bonus_lock_in" && referenceStandard ? <CalculatorReferenceSection busy={isCalculating || invalid} description="Equalised reference after the selected bonus assumption and penny placement." inspectionId="calculators.bonus-lock-in.standard" rows={[{ label: "Lay stake", value: referenceStandard.lay_stake, copyable: true }, { label: "Liability", value: referenceStandard.liability }, { label: "Back wins", value: referenceStandard.back_wins_total }, { label: "Back loses", value: referenceStandard.back_loses_total }]} title="Standard" /> : null}<div className="calculator-advanced-reference-pair">{referenceUnderlay ? <CalculatorReferenceSection busy={isCalculating || invalid} action={<button aria-pressed={inputs.strategy === "Underlay"} className="button-link" disabled={isCalculating || invalid} onClick={() => updatePatch({ strategy: "Underlay", manualLayStake: "" })} type="button">Use Underlay plan</button>} description="Lower lay reference; penny placement can leave a small residual." inspectionId={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.underlay" : "calculators.matched-betting.underlay"} rows={[{ label: "Lay stake", value: referenceUnderlay.lay_stake, copyable: true }, { label: "Liability", value: referenceUnderlay.liability }, { label: "Back wins", value: referenceUnderlay.back_wins_total }, { label: "Back loses", value: referenceUnderlay.back_loses_total }]} title="Underlay" tone="underlay" /> : <CalculatorReferenceSection busy={isCalculating || invalid} description="No valid non-negative endpoint exists for these inputs." inspectionId="calculators.matched-betting.underlay-unavailable" rows={[]} title="Underlay" tone="underlay" />}{referenceOverlay ? <CalculatorReferenceSection busy={isCalculating || invalid} action={<button aria-pressed={inputs.strategy === "Overlay"} className="button-link" disabled={isCalculating || invalid} onClick={() => updatePatch({ strategy: "Overlay", manualLayStake: "" })} type="button">Use Overlay plan</button>} description="Upper lay reference; penny placement can leave a small residual." inspectionId={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.overlay" : "calculators.matched-betting.overlay"} rows={[{ label: "Lay stake", value: referenceOverlay.lay_stake, copyable: true }, { label: "Liability", value: referenceOverlay.liability }, { label: "Back wins", value: referenceOverlay.back_wins_total }, { label: "Back loses", value: referenceOverlay.back_loses_total }]} title="Overlay" tone="overlay" /> : <CalculatorReferenceSection busy={isCalculating || invalid} description="No valid non-negative endpoint exists for these inputs." inspectionId="calculators.matched-betting.overlay-unavailable" rows={[]} title="Overlay" tone="overlay" />}</div><section className="calculator-custom-reference-group" data-pd-id="calculators.matched-betting.custom-group">{referenceCustom ? <CalculatorReferenceSection busy={isCalculating || invalid} description="Editable planned reference, not evidence of an actual matched lay." inspectionId="calculators.matched-betting.custom" rows={[{ label: "Lay stake", value: referenceCustom.lay_stake }, { label: "Liability", value: referenceCustom.liability }, { label: "Back wins", value: referenceCustom.back_wins_total }, { label: "Back loses", value: referenceCustom.back_loses_total }]} title="Custom Lay" tone="custom" action={<div className="calculator-custom-input-row"><Field error={inputs.customLayDraft && !isPositiveAmount(inputs.customLayDraft) ? "Enter a positive Custom lay stake." : null} id="custom-reference-lay" label="Custom Lay" onChange={(value) => updatePatch({ strategy: "Custom", manualLayStake: value, customLayDraft: value })} value={inputs.customLayDraft || referenceCustom.lay_stake} /><CopyableFinancialValue dataPdId="calculators.matched-betting.custom-input-copy" disabled={isCalculating || invalid} label="Custom Lay stake" value={inputs.customLayDraft || referenceCustom.lay_stake} /></div>} /> : null}<SingleLayCustomSlider current={advancedCurrent} maximum={advancedMaximum} maximumText={advancedMaximumText} minimum={advancedMinimum} minimumText={advancedMinimumText} onCommit={(value) => updateBonusCustomStake(Number(value))} onDraft={(value) => updateBonusCustomStake(Number(value))} onMaximumChange={(value) => update("customMaximum", value)} onMinimumChange={(value) => update("customMinimum", value)} /></section></section> : <CalculatorReferenceSection busy={isCalculating || invalid} description="Equalised reference; the reviewed plan remains separate from actual placement." inspectionId="calculators.matched-betting.standard" rows={[{ label: "Lay stake", value: result.selected_lay_stake, copyable: true }, { label: "Liability", value: result.liability }, { label: inputs.betType === "cashback" ? "Without cashback" : "Matched result", value: result.matched_result }]} title="Standard" />}<CalculatorOutcomes busy={isCalculating || invalid} columns={["Bookmaker", "Exchange", "Bonus / cashback"]} inspectionId="calculators.outcomes" rows={result.outcomes.map((outcome, index) => ({ key: outcome.key, label: outcome.label, copyableTotal: true, tone: index === 0 ? "positive" : "exchange", components: [[outcome.bookmaker_component], [outcome.exchange_component], [outcome.promotion_component]], total: outcome.total }))} summary={<span>{inputs.betType === "cashback" ? "Without cashback " : "Matched result "}<CalculatorOutcomeValueDisplay label={inputs.betType === "cashback" ? "Without cashback" : "Matched result"} value={result.matched_result} /></span>} /></div> : null}
  ...
  return <div className="calculator-panel-shell" data-pd-id="calculators.multi-lay.presentation"><div className="calculator-shell">
    <div className="calculator-band calculator-band-primary">
      ...
      <div className="ledger-calculator-mode-bar multi-lay-global-controls" data-pd-id="calculators.multi-lay.global-controls">
        <SelectField disabled={planning} id="multi-backing-type" label="Bet Type" value={inputs.backingType} onChange={(value) => update({ ...inputs, backingType: value as MultiLayInputs["backingType"] })} options={[["normal", "Normal"], ["free_bet_snr", "Free Bet (SNR)"], ["money_back", "Money Back if bet loses"]]} />
        <div className="field-control ledger-calculator-mode-field"><span>Mode</span><CalculatorSegmentedControl ariaLabel="Multi-Lay presentation" onChange={(value) => { const presentationMode = value as MultiLayInputs["presentationMode"]; update({ ...inputs, presentationMode, strategy: presentationMode === "simple" ? "standard" : inputs.strategy }); }} options={[{ value: "simple", label: "Simple" }, { value: "advanced", label: "Advanced" }]} value={inputs.presentationMode} /></div>
      ...
      {!planning || inputs.backingType === "money_back" ? <section className="calculator-segment calculator-segment-offer" data-pd-id="calculators.multi-lay.offer-details"><CalculatorSegmentEyebrow>Offer details</CalculatorSegmentEyebrow><div className="calculator-segment-grid calculator-segment-grid-offer">
        {!planning ? <Field error={multiRangeError(inputs.profitBoostPercent, 0, Infinity, "Enter zero or a positive percentage.")} id="multi-profit-boost" label="Profit Boost (%)" onChange={(value) => update({ ...inputs, profitBoostPercent: value })} value={inputs.profitBoostPercent} /> : null}
      ...
      </div>{result ? <div className="calculator-reference-inline"><span>Effective odds</span><strong>{result.effective_back_odds}</strong></div> : null}</section>
    </div>
    <div className="calculator-band calculator-band-primary calculator-band-single calculator-band-multilay">
      <div className="calculator-panel-card calculator-panel-card-multilay">
        <div className="stack">
          ...
          {inputs.presentationMode === "advanced" ? <section className="calculator-advanced-reference-group" data-pd-id="calculators.multi-lay.advanced">
            <div className="calculator-reference-card-grid">{result?.references.filter((reference) => !planning || reference.strategy !== "overlay").map((reference) => { const title = reference.strategy[0].toUpperCase() + reference.strategy.slice(1); return <CalculatorReferenceSection key={reference.strategy} busy={busy} description={reference.multiplier ? `${title} allocation uses the verified ${reference.multiplier}× source endpoint.` : `No valid ${title.toLowerCase()} endpoint is available for these inputs.`} inspectionId={`calculators.multi-lay.${reference.strategy}`} rows={reference.total_lay_stake ? [{ label: "Lay stake", value: reference.total_lay_stake, copyable: true }] : []} title={title} tone={reference.strategy === "underlay" ? "underlay" : reference.strategy === "overlay" ? "overlay" : "standard"} action={<button aria-pressed={inputs.strategy === reference.strategy} className="button-link" disabled={busy || !reference.multiplier} onClick={() => setStrategy(reference.strategy)} type="button">Use {title} allocation</button>} />; })}</div>
            {!planning ? <section className="calculator-custom-reference-group" data-pd-id="calculators.multi-lay.custom-group"><CalculatorReferenceSection busy={busy} description="Custom allocation multiplies every unrounded Standard lay stake; 1.00× is Standard." inspectionId="calculators.multi-lay.custom" rows={[]} title="Custom Lay" tone="custom" action={<div className="calculator-custom-input-row"><Field error={multiRangeError(inputs.customMultiplier, 0, Infinity, "Enter zero or a positive multiplier.")} id="multi-custom-multiplier" label="Custom multiplier" onChange={(value) => update({ ...inputs, strategy: "custom", customMultiplier: value })} value={inputs.customMultiplier} /><button aria-pressed={inputs.strategy === "custom"} className="button-link" disabled={busy || Boolean(multiRangeError(inputs.customMultiplier, 0, Infinity, "Enter zero or a positive multiplier."))} onClick={() => setStrategy("custom")} type="button">Use Custom allocation</button></div>} />
            {result ? <SingleLayCustomSlider ariaLabel="Custom Multi-Lay multiplier" centre={1} current={current} maximum={maximum} maximumText={inputs.customMaximum || result.default_maximum_multiplier} minimum={minimum} minimumText={inputs.customMinimum || result.default_minimum_multiplier} onCommit={(value) => updateLive({ ...inputs, strategy: "custom", customMultiplier: value })} onDraft={(value) => updateLive({ ...inputs, strategy: "custom", customMultiplier: value })} onMaximumChange={(value) => update({ ...inputs, customMaximum: value })} onMinimumChange={(value) => update({ ...inputs, customMinimum: value })} unit="multiplier" /> : null}</section> : null}
          </section> : null}
