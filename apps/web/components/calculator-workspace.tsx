...
import { formatSportsbookOdds, hasCompleteDecimalInputSyntax, getSportsbookOddsInputError, normalizeCalculatorOddsInput } from "@/lib/sportsbook-odds-input";
...
type Inputs = {
  ...
  commissionUnits: "ratio" | "percent"; cashbackRewardKind: "cash" | "free_bet";
  customMinimum: string; customMaximum: string; customLayDraft: string;
...
type Result = {
  ...
  cashback_credit_face_value: string | null;
  cashback_estimated_retained_value: string | null;
};

const defaults: Inputs = {
  commissionUnits: "ratio", cashbackRewardKind: "cash",
  ...
  promotionValue: "", customMinimum: "", customMaximum: "", customLayDraft: "", bonusTrigger: "Lay Wins", bonusBackingBet: "Normal", retentionPercent: "70", underlayFactor: "0.928", overlayFactor: "1.300",
...
function readInitial(search: URLSearchParams): Inputs {
  ...
  if (next.strategy !== "Standard") next.presentationMode = "Advanced";
  if (next.strategy === "Custom" && !next.customLayDraft) next.customLayDraft = next.manualLayStake;
  if (next.commissionUnits === "percent") next.exchangeCommission = percentageToCommissionRatio(next.exchangeCommission);
  next.commissionUnits = "ratio";
  return next;
}
...
export function CalculatorWorkspace({ popout = false }: { popout?: boolean }) {
  ...
  const lastCalculationStartedAtRef = useRef(0);
  ...
  function update<K extends keyof Inputs>(field: K, value: Inputs[K]) {
    if (field === "betType" && inputs.strategy === "Partial Lay" && inputs.manualLayStake) {
      setError("Review the recorded partial lay before changing offers; it has not been discarded.");
      return;
    }
    ...
    // Preserve the result shell during edits; all result actions are pending-disabled.
    // Validation and request-version guards decide whether a replacement is usable.
    setError(""); setConversion(""); setIsCalculating(true);
  }

  function updatePatch(patch: Partial<Inputs>) {
    if (inputs.strategy === "Partial Lay" && inputs.manualLayStake &&
      ((patch.strategy && patch.strategy !== "Partial Lay") || patch.betType)) {
      setError("Review the recorded partial lay before changing its hedge; it has not been discarded.");
      return;
    }
    requestAbortRef.current?.abort(); requestAbortRef.current = null; requestVersionRef.current += 1;
    setInputs((current) => ({ ...current, ...patch }));
    const validDraft = !Object.values(validate({ ...inputs, ...patch })).some(Boolean);
    setError(""); setConversion(""); setIsCalculating(validDraft);
  }

  function updateBonusCustomStake(value: number) {
    requestAbortRef.current?.abort(); requestAbortRef.current = null; requestVersionRef.current += 1;
    setInputs((current) => ({ ...current, strategy: "Custom", manualLayStake: value.toFixed(2), customLayDraft: value.toFixed(2) }));
  ...
  function updatePresentationMode(mode: Inputs["presentationMode"]) {
    if (mode === "Simple" && inputs.strategy !== "Standard") {
      if (inputs.strategy === "Partial Lay" && inputs.manualLayStake) {
        setError("Recorded partial lay remains active. Review it before returning to Simple.");
        return;
      }
      updatePatch({ presentationMode: mode, strategy: "Standard", manualLayStake: "" });
  ...
  async function calculate() {
    if (invalid) return;
    lastCalculationStartedAtRef.current = Date.now();
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/fund-manager/calculators/matched-betting/preview`, {
        ...
        body: JSON.stringify({
          ...
          cashback_reward_kind: inputs.cashbackRewardKind,
          show_custom_reference: inputs.presentationMode === "Advanced" || inputs.betType === "bonus_lock_in",
          custom_reference_lay_stake: inputs.presentationMode === "Advanced" || inputs.betType === "bonus_lock_in" ? inputs.customLayDraft : "",
        }),
      });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { void calculate(); }, Math.max(0, 120 - (Date.now() - lastCalculationStartedAtRef.current)));
  ...
  const referenceCustom = result?.strategy_references?.find((item) => item.strategy === "Custom");
  const matchedReferenceRows = (reference: NonNullable<typeof referenceStandard>) => [
    { label: "Lay stake", value: reference.lay_stake, copyable: true },
    { label: "Liability", value: reference.liability },
    { label: "Bookmaker wins", value: reference.back_wins_total },
    { label: "Exchange wins", value: reference.back_loses_total },
  ];
  const matchedReferenceCards = [
    { title: "Underlay", reference: referenceUnderlay, tone: "underlay" as const, description: "Underlay favours the bookmaker-win side." },
    { title: "Standard", reference: referenceStandard, tone: "standard" as const, description: "Standard aims to equalise the outcomes." },
    { title: "Overlay", reference: referenceOverlay, tone: "overlay" as const, description: "Overlay favours the exchange-win side." },
  ];
  ...
  const canConvertMatchedResult = Boolean(result) && !invalid && !isCalculating && !(inputs.betType === "bonus_lock_in" && inputs.bonusBackingBet !== "Normal");
  ...
  return <section aria-labelledby="calculator-workspace-title" className={`content-panel stack sportsbook-page-shell${popout ? " calculator-popout-workspace" : ""}`} data-pd-id="calculators.workspace">
    ...
    {family === "matched-betting" ? <div className="calculator-panel-shell"><div className="calculator-shell">
      <div className="calculator-band calculator-band-primary">
        <div className="ledger-calculator-mode-bar calculator-workspace-control-bar">
          <SelectField id="calculator-offer" label="Offer" value={inputs.betType === "free_bet" ? "qualifying" : inputs.betType} onChange={(value) => update("betType", value as BetType)} options={[["qualifying", "Qualifying / Free Bet"], ["bonus_lock_in", "Bonus Lock-In"], ["cashback", "Cashback"], ["profit_boost", "Profit Boost"]]} />
          {inputs.betType === "qualifying" || inputs.betType === "free_bet" ? <SelectField id="bet-type" label="Bet Type" value={qualifyingBackingType} onChange={(value) => updateQualifyingBackingType(value as "Normal" | "SNR" | "SR")} options={[["Normal", "Normal"], ["SNR", "Free Bet SNR"], ...(inputs.freeBetMode === "SR" ? [["SR", "Free Bet SR (legacy saved state)", true] as const] : [])]} /> : null}
          <div className="field-control ledger-calculator-mode-field calculator-presentation-mode-field"><span>Mode</span><CalculatorSegmentedControl ariaLabel="Standard calculator mode" dataPdId="calculators.matched-betting.mode" onChange={updatePresentationMode} options={[{ label: "Simple", value: "Simple" }, { label: "Advanced", value: "Advanced" }]} value={inputs.presentationMode} /></div>
          ...
          {inputs.betType === "cashback" ? <SelectField id="cashback-reward-kind" label="Refund received as" value={inputs.cashbackRewardKind} onChange={(value) => update("cashbackRewardKind", value as Inputs["cashbackRewardKind"])} options={[["cash", "Cash"], ["free_bet", "Free Bet credit"]]} /> : null}
          {inputs.betType === "bonus_lock_in" ? <SelectField id="bonus-trigger" label={inputs.betType === "bonus_lock_in" ? "Bonus Applied If Bet" : "Cashback applies if bet"} value={inputs.bonusTrigger} onChange={(value) => update("bonusTrigger", value as Inputs["bonusTrigger"])} options={[["Lay Wins", "Loses"], ...(inputs.bonusTrigger === "Back Wins" ? [["Back Wins", "Wins (legacy saved state)", true] as const] : [])]} /> : null}
          {inputs.strategy === "Partial Lay" ? <p className="field-hint" role="status">Recorded partial match: actual amounts remain operational inputs, not a selectable profit strategy.</p> : null}
        ...
        <div className={`form-grid calculator-paired-segments calculator-paired-rows-${pairedFieldRows}`} data-pd-id="calculators.matched-betting.paired-segments">
          ...
          <section className="calculator-segment calculator-segment-lay calculator-paired-segment" style={{ "--calculator-segment-field-rows": layFieldRows, "--calculator-segment-span": layFieldRows + 2 } as CSSProperties}><CalculatorSegmentEyebrow>Lay bet</CalculatorSegmentEyebrow><div className="calculator-segment-grid calculator-paired-segment-fields">
            ...
            <Field error={touched.exchangeCommission ? errors.exchangeCommission : null} id="commission" label="Exchange commission (%)" onChange={(value) => { commissionWasEdited.current = true; update("exchangeCommission", value); }} value={inputs.exchangeCommission} />
          </div></section>
        </div>
        {(showPromotion || inputs.betType === "profit_boost" || inputs.strategy === "Underlay" || inputs.strategy === "Overlay") ? <div className="form-grid">
          {showPromotion ? <Field error={touched.promotionValue ? errors.promotionValue : null} id="promotion-value" label={inputs.betType === "bonus_lock_in" ? "Bonus / refund value" : "Eligible refund amount / cap"} onChange={(value) => update("promotionValue", value)} value={inputs.promotionValue} /> : null}
        ...
        {inputs.betType === "cashback" ? <p className="calculator-section-guidance" data-pd-id="calculators.cashback.terms">Cashback depends on the offer&apos;s eligible finishing position, not every loss. The amount is the eligible refund/cap, not confirmation of receipt.</p> : null}
      ...
      {result ? <div className="calculator-band calculator-band-secondary" data-pd-id="calculators.matched-betting.results">
        {showAdvancedReferences ? <section className="calculator-advanced-reference-group" data-pd-id={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.advanced" : "calculators.matched-betting.advanced"}>
          <div className="calculator-reference-card-grid">{matchedReferenceCards.map(({ title, reference, tone, description }) => <CalculatorReferenceSection busy={isCalculating || invalid} description={reference ? description : "No valid non-negative endpoint exists for these inputs."} inspectionId={`${inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in" : "calculators.matched-betting"}.${title.toLowerCase()}`} key={title} rows={reference ? matchedReferenceRows(reference) : []} title={title} tone={tone} />)}</div>
          <section className="calculator-custom-reference-group" data-pd-id="calculators.matched-betting.custom-group">{referenceCustom ? <CalculatorReferenceSection busy={isCalculating || invalid} description="Custom lets you choose your own planned lay stake." inspectionId="calculators.matched-betting.custom" rows={matchedReferenceRows(referenceCustom).filter((row) => row.label !== "Lay stake")} title="Custom" tone="custom" action={<div className="calculator-custom-input-row"><Field error={inputs.customLayDraft && !isPositiveAmount(inputs.customLayDraft) ? "Enter a positive Custom lay stake." : null} id="custom-reference-lay" label="Lay stake" onChange={(value) => updatePatch({ strategy: "Custom", manualLayStake: value, customLayDraft: value })} value={inputs.customLayDraft || referenceCustom.lay_stake} /><CopyableFinancialValue dataPdId="calculators.matched-betting.custom-input-copy" disabled={isCalculating || invalid} label="Custom lay stake" value={inputs.customLayDraft || referenceCustom.lay_stake} /></div>} /> : null}<SingleLayCustomSlider current={advancedCurrent} maximum={advancedMaximum} maximumText={advancedMaximumText} minimum={advancedMinimum} minimumText={advancedMinimumText} onCommit={(value) => updateBonusCustomStake(Number(value))} onDraft={(value) => updateBonusCustomStake(Number(value))} onMaximumChange={(value) => update("customMaximum", value)} onMinimumChange={(value) => update("customMinimum", value)} /></section>
        </section> : referenceStandard ? <CalculatorReferenceSection busy={isCalculating || invalid} description="Standard aims to equalise the outcomes." inspectionId="calculators.matched-betting.standard" rows={matchedReferenceRows(referenceStandard)} title="Standard" /> : null}<CalculatorOutcomes busy={isCalculating || invalid} columns={["Bookmaker", "Exchange", "Bonus / cashback"]} inspectionId="calculators.outcomes" rows={result.outcomes.map((outcome, index) => ({ key: outcome.key, label: outcome.label, copyableTotal: true, tone: index === 0 ? "positive" : "exchange", components: [[outcome.bookmaker_component], [outcome.exchange_component], [outcome.promotion_component]], total: outcome.total }))} summary={<span>{inputs.betType === "cashback" ? "Without cashback " : "Matched result "}<CalculatorOutcomeValueDisplay label={inputs.betType === "cashback" ? "Without cashback" : "Matched result"} value={result.matched_result} /></span>} /></div> : null}
...
function validate(inputs: Inputs): Record<string, string | null> {
  ...
  return {
    ...
    exchangeCommission: decimalRange(inputs.exchangeCommission, 0, 1, "Enter Commission (%) from 0 to 100, for example 2."),
    ...
    retentionPercent: inputs.betType === "bonus_lock_in" || (inputs.betType === "cashback" && inputs.cashbackRewardKind === "free_bet") ? decimalRange(inputs.retentionPercent, 0, 100, "Enter a percentage from 0 to 100.") : null,
...
type MultiLayInputs = {
  ...
  exchange: string;
  outcomes: Array<{ id: string; label: string; layOdds: string; commission: string; commissionManual?: boolean }>;
};
type MultiLayResult = {
...
function readMultiLay(search: URLSearchParams): MultiLayInputs {
  const fallback: MultiLayInputs = {
    ...
    customMaximum: "",
    exchange: "Smarkets",
    outcomes: [{ id: "outcome-1", label: "Outcome 1", layOdds: "", commission: "0", commissionManual: false }, { id: "outcome-2", label: "Outcome 2", layOdds: "", commission: "0", commissionManual: false }],
  };
  try {
    ...
    return {
      ...
      outcomes: parsed.outcomes.map((outcome, index) => ({ ...outcome, id: outcome.id || `outcome-${index + 1}`, commission: outcome.commission ?? legacy.commission ?? "0" })),
    };
  } catch { return fallback; }
}

export function MultiLayCalculator({ exchanges, onState, search, planning = false }: { exchanges: ExchangeOption[]; onState: (params: URLSearchParams) => void; search: URLSearchParams; planning?: boolean }) {
  ...
  function update(next: MultiLayInputs) {
    if (planning && (next.backingType !== "normal" || Number(next.profitBoostPercent) !== 0 || !["standard", "underlay"].includes(next.strategy))) {
      setError("This saved planning contract supports Normal, no boost, Standard or Underlay only."); return;
    }
    requestAbort.current?.abort(); requestVersion.current += 1; setInputs(next); setResult(null); setError(""); setBusy(false);
  }
  function updateLive(next: MultiLayInputs) { if (planning) { update(next); return; } requestAbort.current?.abort(); requestVersion.current += 1; setInputs(next); setError(""); setBusy(false); }
  ...
  const conversionCompatible = !planning && inputs.backingType === "normal" && Number(inputs.profitBoostPercent) === 0 && (inputs.strategy === "standard" || inputs.strategy === "underlay");
  const primaryBetType = inputs.backingType === "free_bet_snr"
    ? "free_bet_snr"
    : inputs.strategy === "underlay" && inputs.backingType === "normal"
      ? "normal_underlay"
      : "normal";
  const rewardModifier = inputs.backingType === "money_back"
    ? "money_back"
    : Number(inputs.profitBoostPercent) > 0
      ? "profit_boost"
      : "none";
  const effectiveOddsChanged = Boolean(result) && Number.isFinite(Number(result?.effective_back_odds)) &&
    Number.isFinite(Number(inputs.backOdds)) && Math.abs(Number(result?.effective_back_odds) - Number(inputs.backOdds)) > 0.000001;
  function setPrimaryBetType(value: string) {
    if (value === "free_bet_snr") {
      update({ ...inputs, backingType: "free_bet_snr", strategy: "standard", profitBoostPercent: "0" });
      return;
    }
    update({
      ...inputs,
      backingType: "normal",
      strategy: value === "normal_underlay" ? "underlay" : "standard",
      profitBoostPercent: "0",
    });
  }
  function setRewardModifier(value: string) {
    if (value === "money_back") {
      update({ ...inputs, backingType: "money_back", strategy: inputs.strategy === "underlay" ? "standard" : inputs.strategy, profitBoostPercent: "0" });
      return;
    }
    update({
      ...inputs,
      backingType: inputs.backingType === "free_bet_snr" ? "free_bet_snr" : "normal",
      profitBoostPercent: value === "profit_boost" ? (Number(inputs.profitBoostPercent) > 0 ? inputs.profitBoostPercent : "10") : "0",
    });
  }
  const setStrategy = (strategy: MultiLayInputs["strategy"]) => update({ ...inputs, strategy, presentationMode: "advanced" });
  const addOutcome = () => {
    const nextNumber = inputs.outcomes.reduce((highest, outcome) => {
      const match = outcome.id.match(/^outcome-(\d+)$/);
  ...
  return <div className="calculator-panel-shell" data-pd-id="calculators.multi-lay.presentation"><div className="calculator-shell">
    <div className="calculator-band calculator-band-primary calculator-band-single calculator-band-multilay">
      ...
      <section className="calculator-segment calculator-segment-back" data-pd-id="calculators.multi-lay.back"><CalculatorSegmentEyebrow>Back bet</CalculatorSegmentEyebrow><div className="calculator-segment-grid calculator-segment-grid-back">
        ...
        <div className="stack calculator-content-grid">
          <CalculatorTableSection className="multi-lay-input-section calculator-table-section-input" headingAction={<ContextHelp label="About Multi-Lay outcome limits" text="Add up to 20 outcomes." />} inspectionId="calculators.multi-lay.lay-outcomes" title="Lay outcomes" tone="lay">
            ...
            {inputs.outcomes.map((outcome, index) => <tr data-pd-id={`calculators.multi-lay.outcome-${index + 1}`} key={outcome.id}>
              ...
              <td data-label="Commission (%)"><label className="field-control"><span className="sr-only">Outcome {index + 1} commission</span><CommissionInput aria-label={`Outcome ${index + 1} Commission (%)`} aria-invalid={Boolean(commissionError(outcome.commission))} data-pd-id={`calculators.multi-outcome-${index + 1}-commission`} inputMode="decimal" onRatioChange={(value) => updateOutcome(index, "commission", value)} value={outcome.commission} /></label></td>
          ...
          <div className="tracker-nav multi-lay-add-row"><button className="button-link icon-text-action" disabled={inputs.outcomes.length >= 20} onClick={addOutcome} type="button"><span aria-hidden="true" className="material-symbols-outlined">add</span><span>Add outcome</span></button></div>
          </CalculatorTableSection>
          {result ? <section className="multi-lay-primary-result stack-tight" aria-labelledby="multi-lay-result-title" data-pd-id="calculators.multi-lay.result"><div className="multi-lay-result-heading"><span className="eyebrow" id="multi-lay-result-title">Result</span><span className="table-chip">{primaryBetType === "free_bet_snr" ? "Free Bet SNR" : primaryBetType === "normal_underlay" ? "Normal Underlay" : "Normal"}</span></div><div className="multi-lay-result-list">{result.branches.map((branch, index) => <article className="multi-lay-result-row" key={inputs.outcomes[index]?.id ?? branch.label}><strong>{branch.label}</strong><dl className="multi-lay-result-values"><div><dt>Lay stake</dt><dd><CopyableFinancialValue dataPdId={`calculators.multi-lay.outcome-${index + 1}.copyable`} label={`${branch.label} lay stake`} value={branch.lay_stake} /></dd></div><div><dt>Liability</dt><dd><FinancialValue label={`${branch.label} liability`} tone="neutral" value={branch.liability} /></dd></div><div><dt>If this outcome wins</dt><dd><FinancialValue label={`${branch.label} final result`} value={branch.outcome_value} /></dd></div></dl></article>)}</div><dl className="multi-lay-result-summary"><div><dt>Total liability</dt><dd><FinancialValue label="Total liability" tone="neutral" value={result.total_liability} /></dd></div><div><dt>{primaryBetType === "free_bet_snr" ? "Free bet loses" : "Back bet loses"}</dt><dd><FinancialValue label={primaryBetType === "free_bet_snr" ? "Free bet loses result" : "Back bet loses result"} value={result.no_selection_value} /></dd></div><div><dt>Lowest final position</dt><dd><FinancialValue label="Lowest final position" value={result.matched_result} /></dd></div></dl></section> : null}
          {!planning ? <details className="calculator-guidance-disclosure multi-lay-secondary-disclosure" data-pd-id="calculators.multi-lay.reward-modifiers"><summary>Reward modifiers</summary><div className="calculator-secondary-controls"><SelectField id="multi-reward-modifier" label="Reward" value={rewardModifier} onChange={setRewardModifier} options={[["none", "None"], ["profit_boost", "Profit Boost"], ["money_back", "Money Back / refund"]]} />{rewardModifier === "profit_boost" ? <Field error={multiRangeError(inputs.profitBoostPercent, 0, Infinity, "Enter zero or a positive percentage.")} id="multi-profit-boost" label="Profit Boost (%)" onChange={(value) => update({ ...inputs, profitBoostPercent: value })} value={inputs.profitBoostPercent} /> : null}{rewardModifier === "money_back" ? <><Field error={inputs.refundAmount && !isPositiveAmount(inputs.refundAmount) ? "Enter a refund amount greater than zero." : null} id="multi-refund" label="Refund amount" onChange={(value) => update({ ...inputs, refundAmount: value })} value={inputs.refundAmount} /><Field error={multiRangeError(inputs.retentionPercent, 0, 100, "Enter a percentage from 0 to 100.")} id="multi-retention" label="Retention (%)" onChange={(value) => update({ ...inputs, retentionPercent: value })} value={inputs.retentionPercent} /></> : null}</div></details> : null}
          <details className="calculator-guidance-disclosure multi-lay-secondary-disclosure" data-pd-id="calculators.multi-lay.advanced"><summary>Advanced allocation <span className="field-hint">Current: {inputs.strategy[0].toUpperCase() + inputs.strategy.slice(1)}</span></summary><div className="stack-tight"><CalculatorSegmentedControl ariaLabel="Multi-Lay allocation" onChange={setStrategy} options={(planning ? [{ value: "underlay", label: "Underlay" }, { value: "standard", label: "Standard" }] : [{ value: "underlay", label: "Underlay" }, { value: "standard", label: "Standard" }, { value: "overlay", label: "Overlay" }, { value: "custom", label: "Custom" }]) as Array<{ value: MultiLayInputs["strategy"]; label: string }>} value={inputs.strategy} />{result ? <div className="multi-lay-allocation-values" aria-label="Allocation reference values">{result.references.filter((reference) => !planning || reference.strategy !== "overlay").map((reference) => <span key={reference.strategy}><strong>{reference.strategy[0].toUpperCase() + reference.strategy.slice(1)}</strong> {reference.multiplier ? `${reference.multiplier}×` : "Unavailable"}</span>)}</div> : null}{inputs.strategy === "custom" && !planning ? <section className="calculator-custom-reference-group" data-pd-id="calculators.multi-lay.custom"><Field error={multiRangeError(inputs.customMultiplier, 0, Infinity, "Enter zero or a positive multiplier.")} id="multi-custom-multiplier" label="Custom multiplier" onChange={(value) => updateLive({ ...inputs, strategy: "custom", customMultiplier: value })} value={inputs.customMultiplier} />{result ? <SingleLayCustomSlider ariaLabel="Custom Multi-Lay multiplier" centre={1} current={current} maximum={maximum} maximumText={inputs.customMaximum || result.default_maximum_multiplier} minimum={minimum} minimumText={inputs.customMinimum || result.default_minimum_multiplier} onCommit={(value) => updateLive({ ...inputs, strategy: "custom", customMultiplier: value })} onDraft={(value) => updateLive({ ...inputs, strategy: "custom", customMultiplier: value })} onMaximumChange={(value) => update({ ...inputs, customMaximum: value })} onMinimumChange={(value) => update({ ...inputs, customMinimum: value })} unit="multiplier" /> : null}</section> : null}</div></details>
          {result ? <CalculatorOutcomes columns={["Bookmaker", "Exchange legs", "Refund"]} headingAction={<ContextHelp label="About Multi-Lay outcomes" text="Each row is a mutually exclusive final outcome. Exchange components are shown in leg order; retained refund is an estimated future value." />} inspectionId="calculators.multi-lay.outcomes" rows={result.scenarios.map((scenario, index) => ({ key: scenario.key, label: scenario.label, tone: index === 0 ? "primary" as const : "exchange" as const, components: [[scenario.bookmaker_component], scenario.lay_components, [scenario.reward_component]], total: scenario.total, selected: scenario.total === result.reference_result }))} summary={<><span>Maximum exchange exposure <CalculatorOutcomeValueDisplay label="Maximum exchange exposure" value={result.maximum_exchange_exposure} /></span><span>Current reference <CalculatorOutcomeValueDisplay label="Current reference" value={result.reference_result} /></span></>} /> : null}
  ...
  return <div className="calculator-panel-shell" data-pd-id="calculators.sequential-lay.presentation"><div className="calculator-shell">
    ...
    <div className="calculator-band calculator-band-primary calculator-band-single calculator-band-multilay"><div className="calculator-panel-card calculator-panel-card-multilay"><div className="multi-lay-calculator-title-row"><span className="eyebrow">Sequential Lay legs</span></div><div className="stack">
      <div className="multi-lay-grid-wrap"><table className={`data-table dense-calculator-grid sequential-lay-grid${inputs.legs.length > 2 ? " has-remove" : ""}`}><thead><tr><th>Leg</th><th>Exchange</th><th>Lay odds</th><th>Commission</th><th>Lay stake</th><th>Liability</th>{inputs.legs.length > 2 ? <th>Remove</th> : null}</tr></thead><tbody>{inputs.legs.map((leg, index) => { const calculated = result?.legs[index]; return <tr data-pd-id={`calculators.sequential-lay.leg-${index + 1}`} key={index}>
        ...
        <td data-label="Commission (%)"><label className="field-control"><span className="sr-only">Leg {index + 1} commission</span><CommissionInput aria-label={`Leg ${index + 1} Commission (%)`} inputMode="decimal" onRatioChange={(value) => updateLeg(index, { commission: value })} value={leg.commission} /></label></td>
...
function commissionError(value: string) { return hasCompleteDecimalInputSyntax(value) && Number.isFinite(Number(value)) && Number(value) >= 0 && Number(value) <= 1 ? null : "Enter commission from 0% to 100%, for example 2 for 2%."; }
...
function Field({ error, id, inputMode = "decimal", label, onBlur, onChange, supportingText, value }: { error: string | null; id: string; inputMode?: "decimal" | "numeric" | "text"; label: string; onBlur?: () => void; onChange: (value: string) => void; supportingText?: string; value: string }) {
  ...
  const isCommission = id === "commission" || id.endsWith("-commission");
  ...
  return <label className={`field-control${error ? " is-invalid" : ""}`} htmlFor={`calculator-${id}`}><span>{label}</span>{isCommission ? <CommissionInput aria-label={label} aria-describedby={error ? errorId : undefined} aria-invalid={Boolean(error)} data-pd-id={dataPdId} id={`calculator-${id}`} onBlur={onBlur} onRatioChange={onChange} value={value} /> : <input aria-describedby={error ? errorId : undefined} aria-invalid={Boolean(error)} data-pd-id={dataPdId} id={`calculator-${id}`} inputMode={inputMode} onBlur={onBlur} onChange={(event) => onChange(event.target.value)} type="text" value={value} />}{error ? <span className="field-validation-text" id={errorId} role="alert">{isCommission ? "Enter commission from 0% to 100%, for example 2 for 2%." : error}</span> : supportingText && !isCommission ? <span className="field-support-text">{supportingText}</span> : null}</label>;
}
function SelectField({ disabled = false, id, label, onChange, options, value }: { disabled?: boolean; id: string; label: string; onChange: (value: string) => void; options: readonly (readonly [string, string, boolean?])[]; value: string }) {
  const dataPdId = id.startsWith("multi-") || id.startsWith("each-way-") || id.startsWith("sequential-") || id.startsWith("early-") || id.startsWith("accumulator-") || id.startsWith("dutching-") || id.startsWith("blackjack-") || id.startsWith("odds-probability-") ? `calculators.${id}` : `calculators.matched-betting.${id}`;
  return <label className="field-control ledger-calculator-mode-field" htmlFor={`calculator-${id}`}><span>{label}</span><select data-pd-id={dataPdId} disabled={disabled} id={`calculator-${id}`} onChange={(event) => onChange(event.target.value)} value={value}>{options.map(([option, text, optionDisabled]) => <option disabled={optionDisabled} key={option} value={option}>{text}</option>)}</select></label>;
}
function ResultValue({ copyable = false, disabled = false, dataPdId, label, money = true, value }: { copyable?: boolean; disabled?: boolean; dataPdId?: string; label: string; money?: boolean; value: string }) { return <div><dt>{label}</dt><dd>{money ? copyable ? <CopyableFinancialValue disabled={disabled} dataPdId={dataPdId} label={label} value={value} /> : <FinancialValue label={label} value={value} /> : value}</dd></div>; }
