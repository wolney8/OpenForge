  ...
  return <section aria-labelledby="calculator-workspace-title" className={`content-panel stack sportsbook-page-shell${popout ? " calculator-popout-workspace" : ""}`} data-pd-id="calculators.workspace">
    ...
    {family === "matched-betting" ? <div className="calculator-panel-shell"><div className="calculator-shell">
      ...
      {result ? <div className="calculator-band calculator-band-secondary" data-pd-id="calculators.matched-betting.results">
        {showAdvancedReferences ? <section className="stack" data-pd-id={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.advanced" : "calculators.matched-betting.advanced"}>{inputs.betType === "bonus_lock_in" && referenceStandard ? <CalculatorReferenceSection busy={isCalculating || invalid} description="Equalised reference after the selected bonus assumption and penny placement." inspectionId="calculators.bonus-lock-in.standard" rows={[{ label: "Lay stake", value: referenceStandard.lay_stake, copyable: true }, { label: "Liability", value: referenceStandard.liability }, { label: "Back wins", value: referenceStandard.back_wins_total }, { label: "Back loses", value: referenceStandard.back_loses_total }]} title="Standard reference" /> : null}<div className="stack">{referenceUnderlay ? <CalculatorReferenceSection busy={isCalculating || invalid} action={<button className="button-link" disabled={isCalculating || invalid} onClick={() => updatePatch({ strategy: "Underlay", manualLayStake: "" })} type="button">Apply Underlay</button>} description="Lower lay reference; penny placement can leave a small residual." inspectionId={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.underlay" : "calculators.matched-betting.underlay"} rows={[{ label: "Lay stake", value: referenceUnderlay.lay_stake, copyable: true }, { label: "Liability", value: referenceUnderlay.liability }, { label: "Back wins", value: referenceUnderlay.back_wins_total }, { label: "Back loses", value: referenceUnderlay.back_loses_total }]} title="Underlay reference" /> : <CalculatorReferenceSection busy={isCalculating || invalid} description="No valid non-negative endpoint exists for these inputs." inspectionId="calculators.matched-betting.underlay-unavailable" rows={[]} title="Underlay unavailable" />}{referenceOverlay ? <CalculatorReferenceSection busy={isCalculating || invalid} action={<button className="button-link" disabled={isCalculating || invalid} onClick={() => updatePatch({ strategy: "Overlay", manualLayStake: "" })} type="button">Apply Overlay</button>} description="Upper lay reference; penny placement can leave a small residual." inspectionId={inputs.betType === "bonus_lock_in" ? "calculators.bonus-lock-in.overlay" : "calculators.matched-betting.overlay"} rows={[{ label: "Lay stake", value: referenceOverlay.lay_stake, copyable: true }, { label: "Liability", value: referenceOverlay.liability }, { label: "Back wins", value: referenceOverlay.back_wins_total }, { label: "Back loses", value: referenceOverlay.back_loses_total }]} title="Overlay reference" /> : <CalculatorReferenceSection busy={isCalculating || invalid} description="No valid non-negative endpoint exists for these inputs." inspectionId="calculators.matched-betting.overlay-unavailable" rows={[]} title="Overlay unavailable" />}</div>{referenceCustom ? <CalculatorReferenceSection busy={isCalculating || invalid} description="Editable planned reference, not evidence of an actual matched lay." inspectionId="calculators.matched-betting.custom" rows={[{ label: "Lay stake", value: referenceCustom.lay_stake, copyable: true }, { label: "Liability", value: referenceCustom.liability }, { label: "Back wins", value: referenceCustom.back_wins_total }, { label: "Back loses", value: referenceCustom.back_loses_total }]} title="Custom Lay reference" action={<Field error={inputs.customLayDraft && !isPositiveAmount(inputs.customLayDraft) ? "Enter a positive Custom lay stake." : null} id="custom-reference-lay" label="Custom Lay" onChange={(value) => updatePatch({ strategy: "Custom", manualLayStake: value, customLayDraft: value })} value={inputs.customLayDraft || referenceCustom.lay_stake} />} /> : null}<SingleLayCustomSlider current={advancedCurrent} maximum={advancedMaximum} maximumText={advancedMaximumText} minimum={advancedMinimum} minimumText={advancedMinimumText} onCommit={(value) => updateBonusCustomStake(Number(value))} onDraft={(value) => updateBonusCustomStake(Number(value))} onMaximumChange={(value) => update("customMaximum", value)} onMinimumChange={(value) => update("customMinimum", value)} /></section> : null}
