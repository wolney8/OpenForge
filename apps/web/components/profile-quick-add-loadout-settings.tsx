      ...
      {ledgers.map((ledger) => {
        ...
        return (
          <section className="content-subpanel stack quick-actions-ledger-section" data-pd-id={`profile-quick-actions.${ledger}`} key={ledger}>
            <header className="quick-actions-ledger-header"><div><span className="eyebrow">{ledger}</span><h3>Quick Actions</h3></div><button className="modal-primary-button icon-text-action" onClick={() => openCreate(ledger)} type="button"><span aria-hidden="true" className="material-symbols-outlined">add</span><span>Add Action</span></button></header>
            ...
            <div className="quick-actions-group"><strong>Profile Actions</strong>{profileActions.length ? profileActions.map((loadout) => {
              ...
              return <article className="quick-action-row" key={loadout.preset_id}><div><strong>{loadout.label}</strong><small>{loadout.enabled_fields.map((field) => fieldLabels[field] ?? field).join(" · ")}{loadout.availability !== "eligible" && loadout.availability_reason ? ` · ${loadout.availability_reason}` : ""}</small></div><label className="profile-filter-chip"><input checked={loadout.enabled} disabled={busy} onChange={(event) => void updateProfileAction(loadout, { enabled: event.target.checked })} type="checkbox" /><span>Enabled</span></label><label className="profile-filter-chip"><input checked={loadout.is_favourite} disabled={busy || isBlocked || !loadout.enabled || (!loadout.is_favourite && favouriteCount >= 4)} onChange={(event) => void updateProfileAction(loadout, { is_favourite: event.target.checked })} type="checkbox" /><span>Carousel</span></label><div className="table-action-group"><button aria-label={`Edit ${loadout.label}`} className="icon-button" onClick={() => openEdit(loadout)} type="button"><span aria-hidden="true" className="material-symbols-outlined">edit</span></button><button aria-label={`Delete ${loadout.label}`} className="icon-button destructive-icon-button" disabled={busy} onClick={() => void archiveProfileAction(loadout)} type="button"><span aria-hidden="true" className="material-symbols-outlined">delete</span></button></div></article>;
            }) : <p className="field-hint">No profile-owned actions yet.</p>}</div>
          </section>
        );
      })}
      {draft && typeof document !== "undefined" ? createPortal(<QuickActionDialog actionId={editingActionId} draft={draft} fields={schemas[draft.ledger_type] ?? []} onChange={setDraft} onClose={() => { setDraft(null); setEditingActionId(null); }} onSave={() => void saveProfileAction()} ref={dialogRef} saving={savingId === "profile-action"} toggleField={toggleDraftField} />, document.body) : null}
    </section>
  );
}
...
const QuickActionDialog = ({ actionId, draft, fields, onChange, onClose, onSave, saving, toggleField, ref }: { actionId: string | null; draft: ActionDraft; fields: string[]; onChange: (draft: ActionDraft) => void; onClose: () => void; onSave: () => void; saving: boolean; toggleField: (field: string) => void; ref: RefObject<HTMLElement | null> }) => (
  <div className="modal-backdrop modal-backdrop-elevated"><section aria-label={`${actionId ? "Edit" : "Add"} Profile Quick Action`} aria-modal="true" className="modal-panel workflow-editor-modal fund-manager-settings-modal settings-adaptive-modal profile-quick-action-modal" ref={ref} role="dialog" tabIndex={-1}>
    ...
    <div className="workflow-editor-modal-body stack settings-adaptive-modal-body"><div className="form-grid settings-dialog-form-grid"><label className="field-control"><span>Ledger</span><select data-initial-focus disabled={Boolean(actionId)} onChange={(event) => onChange({ ...draft, ledger_type: event.target.value as LedgerType, enabled_fields: [], defaults: {} })} value={draft.ledger_type}>{ledgers.map((ledger) => <option key={ledger}>{ledger}</option>)}</select></label><label className="field-control"><span>Action Label</span><input maxLength={80} onChange={(event) => onChange({ ...draft, label: event.target.value })} value={draft.label} /></label></div><fieldset className="quick-action-field-picker"><legend>Prefill fields</legend><div className="quick-action-field-grid">{fields.map((field) => <label className={`profile-filter-chip${draft.enabled_fields.includes(field) ? " is-selected" : ""}`} key={field}><input checked={draft.enabled_fields.includes(field)} onChange={() => toggleField(field)} type="checkbox" /><span>{fieldLabels[field] ?? field}</span></label>)}</div></fieldset>{draft.enabled_fields.length ? <section className="stack"><h3>Defaults</h3><div className="form-grid settings-dialog-form-grid">{draft.enabled_fields.map((field) => <label className="field-control" key={field}><span>{fieldLabels[field] ?? field}</span><input onChange={(event) => onChange({ ...draft, defaults: { ...draft.defaults, [field]: event.target.value } })} value={draft.defaults[field] ?? ""} /></label>)}</div></section> : null}{draft.ledger_type === "Sportsbook" || draft.ledger_type === "Free Bets" ? <section className="content-subpanel stack"><div><span className="eyebrow">Opportunity</span><h3>Reusable prospect</h3><p className="field-hint">Create a prospect only when selected from the Profile Opportunity queue.</p></div><label className="profile-filter-chip"><input checked={draft.defaults.opportunityEnabled === "true"} onChange={(event) => onChange({ ...draft, defaults: { ...draft.defaults, opportunityEnabled: String(event.target.checked), opportunityKind: draft.defaults.opportunityKind || (draft.ledger_type === "Free Bets" ? "Free Bet" : "Reload"), opportunityRecurrence: draft.defaults.opportunityRecurrence || "One Off" } })} type="checkbox" /><span>Use as opportunity</span></label>{draft.defaults.opportunityEnabled === "true" ? <div className="form-grid settings-dialog-form-grid"><label className="field-control"><span>Opportunity Type</span><select onChange={(event) => onChange({ ...draft, defaults: { ...draft.defaults, opportunityKind: event.target.value } })} value={draft.defaults.opportunityKind || (draft.ledger_type === "Free Bets" ? "Free Bet" : "Reload")}><option>Reload</option><option>Signup</option><option>Free Bet</option></select></label><label className="field-control"><span>Recurrence</span><select onChange={(event) => onChange({ ...draft, defaults: { ...draft.defaults, opportunityRecurrence: event.target.value } })} value={draft.defaults.opportunityRecurrence || "One Off"}><option>One Off</option><option>Weekly</option></select></label><label className="field-control field-span-2"><span>Opportunity Notes</span><textarea maxLength={1000} onChange={(event) => onChange({ ...draft, defaults: { ...draft.defaults, opportunityNotes: event.target.value } })} value={draft.defaults.opportunityNotes || ""} /></label></div> : null}</section> : null}</div>
