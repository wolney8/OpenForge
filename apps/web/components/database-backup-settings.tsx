  ...
  return (
    <>
      <section className="content-panel stack" data-pd-id="database-backups.section">
        <div className="sportsbook-page-header"><div><span className="eyebrow">Local data protection</span><h2>Database Backups</h2></div></div>
        ...
        <div className="table-toolbar settings-table-toolbar"><label className="field-control table-search-field"><span>Search backups</span><input aria-label="Search database backups" onChange={(event) => { setSearch(event.target.value); setPage(1); }} type="search" value={search} /></label><div className="settings-table-filter-group"><label className="field-control table-filter-field"><span>State</span><select aria-label="Filter database backup state" onChange={(event) => { setStatusFilter(event.target.value as typeof statusFilter); setPage(1); }} value={statusFilter}><option>All</option><option>Verified</option><option>Verification Failed</option></select></label><button aria-haspopup="dialog" className="modal-primary-button icon-text-action" data-pd-id="database-backups.open" onClick={() => setIsOpen(true)} ref={openButtonRef} type="button"><span aria-hidden="true" className="material-symbols-outlined">backup</span><span>Manage Database Backups</span></button></div></div>
