            ...
            <div className="tracker-nav spreadsheet-review-actions">
              {confirmDeleteBatchId === activeBatch.import_batch_id ? (
                <div className="spreadsheet-delete-confirm" data-pd-id={`spreadsheet-transfer.delete-confirm.${activeBatch.import_batch_id}`} role="group" aria-label={`Confirm delete review ${activeBatch.source_filename}`}>
                  ...
                  <button className="button-link compact-action destructive-action" disabled={isLoading} onClick={() => deleteBatch(activeBatch)} type="button">{isLoading ? <span aria-hidden="true" className="button-spinner" /> : <span aria-hidden="true" className="material-symbols-outlined">delete</span>}<span>{isLoading ? "Deleting" : "Delete"}</span></button>
