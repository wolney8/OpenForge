              ...
              {!isLoading && !editing && !isCreating ? (
                <>
                  <div className="table-toolbar settings-table-toolbar fund-manager-authority-toolbar">
                    ...
                    <div className="settings-table-filter-group"><label className="field-control table-filter-field"><span>List</span><select data-pd-id="fund-manager-authorities.list-filter" onChange={(event) => { setTypeFilter(event.target.value); setPage(1); }} value={typeFilter}>{Object.entries(authorityLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button className="button-link icon-text-action" data-pd-id="fund-manager-authorities.add" onClick={openCreate} type="button"><span aria-hidden="true" className="material-symbols-outlined">add</span><span>Add Value</span></button></div>
                  </div>
                  <LedgerPagination ariaLabel="Tracker Lists" currentPage={page} onPageChange={setPage} onPageSizeChange={(next) => { setPageSize(next); setPage(1); }} pageCount={pageCount} pageSize={pageSize} position="top" totalRows={sortedRows.length} />
