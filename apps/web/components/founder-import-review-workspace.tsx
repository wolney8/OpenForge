  ...
  return <>
    ...
    <section className="content-panel stack founder-import-review" data-pd-id="founder-import-review.workspace">
      ...
      <section className="stat-card-grid import-review-stat-grid import-review-compact-stats" aria-label="Import review status">
        ...
        <article className="stat-card"><span className="eyebrow">P&amp;L impact</span><strong><FinancialValue value={workspace.reconciliation.pnl_impact} /></strong><span>{pnlImpactIsZero ? "£0.00 change to imported P&L" : `${workspace.reconciliation.pnl_impact_items.length} review decisions change imported P&L`}</span></article>
      </section>
      {!pnlImpactIsZero ? <details className="content-subpanel stack-tight import-impact-details" open><summary>Review decisions affecting imported P&amp;L</summary>{workspace.reconciliation.pnl_impact_items.map((item) => <div className="workflow-panel-header" key={item.item_id}><span>{item.source_sheet} row {item.source_row} · {item.action.replaceAll("_", " ")}</span><FinancialValue value={item.value} /></div>)}</details> : null}
      {annualReconciliation ? <details className="content-subpanel stack-tight import-financial-reconciliation">
        ...
        <div className="table-scroll"><table className="data-table"><thead><tr><th scope="col">Period</th><th scope="col">Workbook report</th><th scope="col">Plum Duff equivalent</th><th scope="col">Settled / realised</th><th scope="col">Open current</th><th scope="col">Difference</th></tr></thead><tbody>{(["week", "month", "year"] as const).map((period) => {
          ...
          return <FinancialValueReplayRow key={period}><td><strong>{period[0].toLocaleUpperCase() + period.slice(1)}</strong><span className="table-status">{reconciliation.period_key}</span></td><td><FinancialValue value={reconciliation.workbook_report.total ?? "0.00"} /></td><td><FinancialValue value={reconciliation.plum_duff_from_mapped_rows.total} /></td><td><FinancialValue value={reconciliation.financial_views.realised_settled_pnl.total} /></td><td><FinancialValue value={reconciliation.financial_views.open_current_worst_case_pnl.total} /></td><td><FinancialValue value={reconciliation.difference ?? "0.00"} /></td></FinancialValueReplayRow>;
        })}</tbody></table></div>
      </details> : null}
      ...
      <LedgerTableScroll dataPdId="founder-import-review.table-scroll">
        <table className="data-table import-review-table">
          ...
          <tbody>{rows.length ? rows.map((item) => {
            ...
            return <FinancialValueReplayRow aria-selected={isSelected} className={isSelected ? "is-selected-row" : undefined} key={item.item_id} onClick={(event) => { if (!rowInteractionTarget(event.target)) toggleSelected(item.item_id); }} onKeyDown={(event) => { if (event.key === " " && !rowInteractionTarget(event.target)) { event.preventDefault(); toggleSelected(item.item_id); } }} tabIndex={0}>
              ...
              <td><div className="table-cell-stack table-cell-stack-centered">{item.context.pnl ? <FinancialValue value={item.context.pnl} /> : "—"}<span className="table-status">{item.calculation_provenance.replaceAll("_", " ")}</span></div></td>
      ...
      {approvedReadyStatus && finalSummary ? <section className="content-subpanel stack" data-pd-id="profile-import.final-summary">
        ...
        <div className="stat-card-grid import-review-stat-grid import-review-compact-stats" aria-label="Final import counts">
          ...
          <article className="stat-card"><span className="eyebrow">Open current P&amp;L</span><strong><FinancialValue value={finalSummary.financial.open_current_pnl} /></strong><span>Worst-case/current cash</span></article>
          <article className="stat-card"><span className="eyebrow">Settled P&amp;L</span><strong><FinancialValue value={finalSummary.financial.settled_pnl} /></strong><span>Realised source value</span></article>
        </div>
      ...
      {postImportReport ? <section className="content-subpanel stack" data-pd-id="profile-import.reconciliation" id="profile-import-reconciliation">
        ...
        <details open><summary>Financial reconciliation</summary><div className="table-scroll"><table className="data-table"><thead><tr><th scope="col">Period</th><th scope="col">Workbook / Dry Run</th><th scope="col">Post Import</th><th scope="col">Difference</th></tr></thead><tbody>{Object.entries(postImportReport.financial_reconciliation.periods).map(([period, values]) => <FinancialValueReplayRow key={period}><td>{period}</td><td><FinancialValue value={values.workbook_dry_run} /></td><td><FinancialValue value={values.post_import} /></td><td><FinancialValue value={values.difference} /></td></FinancialValueReplayRow>)}</tbody></table></div></details>
        <details><summary>Financial views</summary><div className="table-scroll"><table className="data-table"><thead><tr><th scope="col">Measure</th><th scope="col">Expected</th><th scope="col">Actual</th><th scope="col">Difference</th></tr></thead><tbody>{Object.entries(postImportReport.financial_reconciliation.views).map(([name, values]) => typeof values === "string" ? <FinancialValueReplayRow key={name}><td>{name.replaceAll("_", " ")}</td><td colSpan={3}><FinancialValue value={values} /></td></FinancialValueReplayRow> : <FinancialValueReplayRow key={name}><td>{name.replaceAll("_", " ")}</td><td><FinancialValue value={values.expected} /></td><td><FinancialValue value={values.actual} /></td><td><FinancialValue value={values.difference} /></td></FinancialValueReplayRow>)}</tbody></table></div></details>
