...
test("Sportsbook row click opens the editor as a modal dialog", async ({ page }) => {
  ...
  await expect(tabRail.getByRole("tab", { name: /Matching/ })).toBeVisible();
  ...
  await expect(editorDialog.locator(".editor-summary-pill-value")).toHaveCount(0);
  ...
  await tabRail.getByRole("tab", { name: /Matching/ }).click();
  ...
  expect(Math.abs((footerBox?.width ?? 0) - (dialogBox?.width ?? 0))).toBeLessThan(2);
  ...
  await expect(editorDialog.locator('[data-pd-id="sportsbook.editor.actions"]').getByRole("button", { name: "Close sportsbook editor" })).toBeVisible();
});
...
test("Sportsbook settled edit state uses Edit, Save Edits, and Revert without the legacy header action", async ({
  ...
  try {
    ...
    await expect(footerPrimaryActions.getByRole("button", { name: "Edit", exact: true })).toBeVisible();
    await expect(footerPrimaryActions.getByRole("button", { name: "Save Edits" })).toHaveCount(0);

    await editorDialog.getByRole("button", { name: "Revert" }).click();
    ...
    await resultSelect.selectOption("Lay Won");
    await expect(footerPrimaryActions.getByRole("button", { name: "Edit", exact: true })).toBeVisible();
    await footerPrimaryActions.getByRole("button", { name: "Edit", exact: true }).click();
    await expect(footerPrimaryActions.getByRole("button", { name: "Save Edits" })).toBeDisabled();
