...
- where the user encounters this value
- which workflow step triggers it
- which route or screen shows it
- whether it is entry-time, open-state, settlement-time, or reporting-time logic
...
- why it exists
- whether it came from workbook behaviour or later interpretation
...
List deterministic fixtures needed to prove this calculation.
...
- open/pending case
- settled/final case
- void/cancelled case if relevant
- manual override case if relevant
- profile isolation case

## 17. Test cases

List automated tests required.
...
- exact equality expected or not
- allowed tolerance if decimals or currency conversions apply
- why that tolerance is acceptable
...
- tooltip/help text requirements
- whether projected/current and final/settled values must both be visible
- formatting rules
- warning states for overrides or stale/open values

## 20. Audit trail requirements

State what must be recorded for later review:
