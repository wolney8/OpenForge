...
| `FINANCIAL-MOTION-001` | Presentation / true shared digit odometer for read-only signed money | `PARTIAL locally`: shared `FinancialValue` now uses clipped digit strips, staggered transforms, initial sign direction, later delta direction, click replay, stable punctuation and reduced-motion suppression; broader surface rollout and user/hosted acceptance remain pending | [Presentation contract](../contracts/financial-value-presentation-contract.md); [fixture spec](../fixture-specs/financial-value-presentation-fixture-spec.md); [#105](https://github.com/wolney8/OpenForge/issues/105) |
