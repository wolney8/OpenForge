...
| Notification consistency | M15 Platform Experience | `docs/workflows/fund-manager-notification-centre-workflow-contract.md` | `tests/fixtures/fund-manager-notification-centre-fixtures.json` | `NOTIFICATION-FIX-001` COMPLETE locally 2026-09-06: durable clear tombstones are monotonic and user-scoped, failed persistence is visible, and stale reads cannot overwrite a completed clear. API 4/4, client 264/264, focused Playwright 2/2 and isolated authenticated browser/API 1/1 passed. Hosted acceptance remains required. Current sources remain `fund_manager_only`; subscriber delivery is separate scope. |
...
| Strict numerical entry | M14 Calculator Workspace or sportsbook enhancement milestone | `sportsbook-odds-input-v1` in the Sportsbook lifecycle contract | `tests/fixtures/sportsbook-odds-input-fixtures.json` | `PD-FUTURE-020` and the Sportsbook slice of `PD-FUTURE-022` implemented locally; other calculator/form surfaces remain pending. `PD-FUTURE-019` payout-derived reference odds is next; `PD-FUTURE-021` field-specific money/rate validation remains design-pending. |
