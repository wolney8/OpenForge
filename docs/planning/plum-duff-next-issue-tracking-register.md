...
| #62 | Google initiation, state/callback/denial/session recovery pass locally | IMPLEMENTED ENGINEERING SIDE AFTER CP-025 LIVE CONFIG REPAIR; 3010 reaches Google with the exact local callback, while fresh owner completion and hosted callback remain unverified |
...
| ID | Requested outcome | Current state |
|---|---|---|
| PQA-J18 / #109 / #49 / #80 | Prove Account access and award lineage as one authenticated portable workbook journey | PASS LOCALLY: six-sheet import, structured access evidence, eligibility, award retry/settlement, £7.18 report, export and portable restore/native-ID remap pass |
| PQA-J16 / #76 / #77 | Search, retain a Profile action and reach the correctly scoped editor without bypassing rules | PASS LOCALLY for the canonical bounded journey: keyboard/stale-response search and Profile Quick Action save/reopen/prefill/validation pass; full #76 command menu and #77 multi-Profile opportunity scope remain separate |
...
#62 and #80 without double-counting them. Coverage is 68/87 assessments, 22/24 journeys, 18/27
competitor cells and 77/133 requirements. No GitHub closure, main merge, push or hosted state is
inferred.
...
| `LOGIN-RELIABILITY-001` | Fix / Google sign-in must visibly initiate from the canonical local origin | `NEEDS OWNER VERIFICATION`: CP-025 repaired the normal-owner environment loss; real 3010 initiation reaches Google's sign-in surface with the exact callback and no URI mismatch, deterministic callback/session tests pass, and Will's fresh completion/return remains pending | Current recovery request; [#62](https://github.com/wolney8/OpenForge/issues/62) |
...
| `ACCOUNTS-IMPORT-ACCESS-001` | Import gap / preserve lifecycle, stake access and promo access as distinct semantics | `INTEGRATED LOCALLY`: canonical Stake/Promo enums, structured restriction evidence, precedence, import/export/restore and Account/Quick Action consumers pass together in the CP-020 authenticated workbook/award journey. Historical `LastPromoUsed` remains derived/provenance-aware rather than fabricated | [Accounts field map](../contracts/accounts-import-field-map-contract.md); [#109](https://github.com/wolney8/OpenForge/issues/109) |
...
| `FINANCIAL-ANALYTICS-EXPLORER-001` | Feature / interactive Profile and authorised combined financial time-series and reusable Reports chart explorer | `PARTIAL / FIRST TWO SLICES INTEGRATED LOCALLY`: focusable point inspection and reconciled underlying-record drilldown pass with keyboard/pointer and retained context. Module/metric/granularity policy and user-created preset persistence remain separately planned; financial measures stay distinct | [#111](https://github.com/wolney8/OpenForge/issues/111); separate data-source work [#106](https://github.com/wolney8/OpenForge/issues/106) |
