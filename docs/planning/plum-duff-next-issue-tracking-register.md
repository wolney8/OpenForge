...
| `NOTIFICATION-LAYOUT-001` | Visual fix / keep Notification History Search, Type, Status, and actions contained, with Search/Type/Status aligned independently of the action row | `NEEDS VERIFICATION` at `630bde8`; Will rejected the first contained layout because Search was displaced by wrapped actions; the grouping defect is now reproduced and corrected with focused alignment/containment/theme tests and rendered synthetic inspection; #100 stays open for Will's recheck | Supplied screenshots and acceptance criteria; [#100](https://github.com/wolney8/OpenForge/issues/100), related bounded review [#92](https://github.com/wolney8/OpenForge/issues/92) |
