...
| `FINANCIAL-MOTION-001` | Presentation / true shared digit odometer for read-only signed money | `NEEDS VERIFICATION locally`: shared `FinancialValue` preserves static geometry and canonical clipboard text, includes neutral-zero and deterministic hover/click/group replay, and is governed by the persisted default-On Fund Manager motion preference with reduced-motion override. Targeted dashboard/command rail, Combined Analytics, tracker summary, ledger/report row, Account summary, drawer, fee and Extra Places surfaces use the shared primitive. Future surfaces remain prospective rollout; user/hosted acceptance is pending | [Presentation contract](../contracts/financial-value-presentation-contract.md); [fixture spec](../fixture-specs/financial-value-presentation-fixture-spec.md); [#105](https://github.com/wolney8/OpenForge/issues/105) |
