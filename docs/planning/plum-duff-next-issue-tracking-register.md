...
| `CALCULATOR-BLACKJACK-001` | Decision support / Simulation, Free Play and Live Play Blackjack sessions | `CODE-VERIFIED — USER RECHECK PENDING`: strategy behaviour is retained. Live Play now totals Gross Staked from actual committed actions and Gross Returned/Net P&L only when every completed hand has an entered Actual Return; reviewed starting/ending-balance result and the difference remain separate. Optional Fixed stake cap / Use winnings limits warn and require acknowledgement without placing or blocking a wager. Rebet, Rebet & Deal and Double & Deal prepare the next calculator hand; Last Hand briefly auto-collapses or remains manually open until initial cards are entered. The additive `blackjack-session-v1` snapshot retains running totals and limit provenance for the existing single-Profile #36 bridge. Wider #40 acceptance remains open. | `blackjack-session-v1`, `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), Casino semantics [#78](https://github.com/wolney8/OpenForge/issues/78), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `CODE-VERIFIED — USER RECHECK PENDING`: top controls now use explicit four-row grid placement: Session Mode/Reset Hand, count/Surrender, Soft 17, then Deal Again. Convertible-session financial and play-limit controls remain above the aligned Dealer/Player table. Running summary and reconciliation values use shared FinancialValue; Last Hand contains bounded next-hand controls and explicit limit acknowledgement. Existing strategy and #36 conversion architecture are unchanged. | [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
