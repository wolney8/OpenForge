...
| `FINANCIAL-MOTION-001` | Presentation / true shared digit odometer for read-only signed money | `PARTIAL locally`: shared `FinancialValue` now sizes clipped digits from inherited tabular glyphs and preserves static currency spacing, typography, baseline and natural width. Targeted active dashboard/point/command rails, Combined Analytics, tracker summaries, ledgers, calculator results, Account summaries, side panels and fee dialogs use the shared component. Legacy/non-active shells, editable/read-only form controls, exports, and future surfaces remain intentionally static or future rollout; user/hosted acceptance remains pending | [Presentation contract](../contracts/financial-value-presentation-contract.md); [fixture spec](../fixture-specs/financial-value-presentation-fixture-spec.md); [#105](https://github.com/wolney8/OpenForge/issues/105) |
