...
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack money fields, contextual help and Player-centric actions | `CODE-VERIFIED — USER RECHECK PENDING`: the shared financial input owns centred £ geometry and bounded numeric width; routine helper rows moved to anchored accessible help; Session mode is compact; card labels share calculator typography; suggested actions call the existing player-action handler. Strategy and `blackjack-session-v1` semantics are unchanged. | [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
