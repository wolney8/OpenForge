...
| `CALCULATOR-BLACKJACK-001` | Decision support / standalone Blackjack basic-strategy reference | `PROVEN locally — CORE`: published hard/soft/pair matrices, surrender and dealer Soft-17 controls return deterministic Hit/Stand/Double/Split/Surrender/Bust guidance; focused API/browser checks passed with no wager/P&L/write path. User/hosted acceptance remains pending. | `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40) |
