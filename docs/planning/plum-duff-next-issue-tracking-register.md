...
| `CALCULATOR-BLACKJACK-001` | Decision support / interactive standalone Blackjack basic-strategy session | `PROVEN locally — CORE; USER/HOSTED ACCEPTANCE PENDING`: one fast rank-only picker preserves the engine's exact `A`, `2`–`10`, `J`, `Q`, `K` contract; decorative suits never enter state. Surrender uses the M3 Switch. Soft 17 defaults to honest Unknown, showing one common recommendation or both S17/H17 moves when they differ. Stake defaults to £0 with £0.50/£1 quick choices; authoritative logout/expiry clears active/history/session state while valid refresh retains it. Prior matrix parity, legal actions, one split and zero-write evidence remain. DAS is assumed from the published matrix; resplitting and outcome/P&L simulation remain uninferred. | `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
