...
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `RENDERED FIX — WILL RECHECK PENDING`: the latest #40/#92 manual regressions are corrected through shared patterns. Compact card/banner undo targets retain identical geometry on hover; Reset Session now shares the main Reset Hand action group and is removed from History. Pending/recommendation/current-hand content uses one persistent 480ms window with reduced-motion override. History puts its chevron inside HAND, allocates remaining desktop width to OUTCOME, and renders expanded detail in a full-width row; half/narrow records stack without clipping or page overflow. Existing Last Hand, settlement, payout, persistence and `blackjack-session-v1`/#36 provenance behaviour remains unchanged. | [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
