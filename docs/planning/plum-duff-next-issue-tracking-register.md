...
|---|---|---|---|
| `CALCULATOR-WORKSPACE-001` | Feature / Fund Manager standalone calculator workspace | `CORE FAMILY COVERAGE PROVEN locally; user acceptance PENDING RECHECK`: Standard, Multi-Lay, Extra Place / Each Way, Sequential Lay, Early Payout / 2UP, Multiples / Accumulator, Simple Dutching, Odds / Probability and Blackjack Strategy are implemented reference-only families. Optional specialised extensions remain explicit below; #36 is next. | Calculator workspace workflow/fixtures; [#35](https://github.com/wolney8/OpenForge/issues/35), contracts/fixtures [#37](https://github.com/wolney8/OpenForge/issues/37), advanced catalogue [#38](https://github.com/wolney8/OpenForge/issues/38), Sequential Lay [#39](https://github.com/wolney8/OpenForge/issues/39), later bridge [#36](https://github.com/wolney8/OpenForge/issues/36) |
...
| `CALCULATOR-BLACKJACK-001` | Decision support / interactive standalone Blackjack basic-strategy session | `PROVEN locally — CORE; USER/HOSTED ACCEPTANCE PENDING`: compact M3 Switch controls default to no surrender and dealer stands on Soft 17, with accessible game-rule help. One fast rank-only picker preserves the engine's exact `A`, `2`–`10`, `J`, `Q`, `K` contract; shared portrait card faces keep decorative suits out of state. Dealer/Player areas, Card 3+ continuation, prominent recommendations/legal actions and upper hand count follow the accepted calculator system. Stake/session/logout behavior and prior matrix parity, one split and zero-write evidence remain. DAS is assumed from the published matrix; resplitting and outcome/P&L simulation remain uninferred. | `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
