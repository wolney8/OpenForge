...
| `CALCULATOR-BLACKJACK-001` | Decision support / Simulation, Free Play and Live Play Blackjack sessions | `PROVEN LOCALLY — USER RECHECK PENDING`: A–I acceptance behaviours passed focused strategy/session tests plus rendered desktop, dual-pane, narrow, light/dark and refresh paths. Strategy behaviour is unchanged. Live/Free hands preserve actual committed stake, calculated-or-entered return provenance and per-hand/running result. Natural Blackjack payout defaults explicitly to 1:1 and supports 3:2, 6:5, 2:1 or validated Custom; ordinary Win remains 1:1. `blackjack-session-v1` retains payout provenance and additive cash/credit hand summaries for the existing single-Profile #36 bridge. #40 remains open. | `blackjack-session-v1`, `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), Casino semantics [#78](https://github.com/wolney8/OpenForge/issues/78), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `PROVEN LOCALLY — USER RECHECK PENDING`: Last Hand uses the shared animated full-width disclosure for click/automatic transitions without a side track or React `inert` warning. UP-CARD and every selected CARD N expose the label-anchored, overlaid shared centred undo control. Money-mode history exposes aligned committed STAKED, RETURNED and NET P&L without adding Simulation finance; stake-known rounds offer Player-header Rebet & Deal Again and emphasized Double & Deal Again only. | [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
