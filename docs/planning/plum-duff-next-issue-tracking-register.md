...
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack fields and Player action composition | `CODE-VERIFIED — USER RECHECK PENDING`: the single recommendation/action panel is a full-width strip above both Dealer and Player, aligned to the combined row edges at desktop, narrow and text-scaled widths. Completed-hand status/outcome controls now form a compact aligned group beside Current Hand; one outcome activation records exactly one History entry and starts the blank next hand while preserving session settings. Blocked Session mode changes expose their reset requirement directly below the deterministic select. Geometry, focus, themes and interaction checks pass. Strategy, cards and `blackjack-session-v1` are unchanged. | [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
