...
| `CALCULATOR-BLACKJACK-001` | Decision support / interactive standalone Blackjack basic-strategy session | `CODE-VERIFIED — USER RECHECK PENDING`: focused rendered checks confirm Deal Again/count left, rule switches central and Reset Hand right; one responsive size/aspect token now governs enlarged picker/dealer/player/back cards, and theme-specific charcoal/high-contrast cards replace dark-mode glare. Proven matrices/rank/callback/session semantics remain unchanged. DAS remains the published matrix assumption; resplitting and casino P&L simulation remain uninferred; user/hosted acceptance is pending. | `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
