...
| `CALCULATOR-BLACKJACK-001` | Decision support / Simulation, Free Play and Live Play Blackjack sessions | `CODE-VERIFIED — USER RECHECK PENDING`: strategy behaviour is retained. Live Play keeps only per-hand Stake and Actual Return in the high-frequency surface and presents Gross Staked, Gross Returned, Net P&L and Remaining Buffer as shared FinancialValue summaries. Slower Activity Source, Table Type, balance and play-limit fields now use one bounded Session Setup modal; Free Play credit/result setup follows the same pattern. The unchanged `blackjack-session-v1` state path and deterministic snapshot retain all relocated values for the existing single-Profile #36 bridge. Wider #40 acceptance remains open. | `blackjack-session-v1`, `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), Casino semantics [#78](https://github.com/wolney8/OpenForge/issues/78), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `CODE-VERIFIED — USER RECHECK PENDING`: Deal Again now uses the Player header's explicit left/right grid, reflowing safely at narrow container widths instead of consuming an upper control row. Session Setup reuses the established settings-modal shell; the main surface retains only mode-appropriate high-frequency inputs and compact running summaries. Dealer/Player alignment, strategy, financial semantics and #36 bridge architecture remain unchanged. | [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
