...
| `CALCULATOR-BLACKJACK-001` | Decision support / Simulation, Free Play and Live Play Blackjack sessions | `CODE-VERIFIED — USER RECHECK PENDING`: strategy behaviour is unchanged. Live/Free hands now preserve explicit stake, actual committed stake, calculated-or-entered return provenance and per-hand/running result. Natural Blackjack derives only after the user selects 3:2 or 6:5; ordinary outcome returns use the signed half-up contract and an entered Actual Return remains authoritative. `blackjack-session-v1` retains payout provenance and additive cash/credit hand summaries for the existing single-Profile #36 bridge. Wider #40 acceptance remains open. | `blackjack-session-v1`, `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), Casino semantics [#78](https://github.com/wolney8/OpenForge/issues/78), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `CODE-VERIFIED — USER RECHECK PENDING`: Last Hand is now the shared full-width disclosure between recommendation and the unchanged Dealer/Player row, with no side control, Keep open action or grid track. It briefly opens, auto-collapses only before manual interaction, and contains Rebet/Rebet & Deal/Double & Deal for Live and Free Play. Session History uses bounded responsive details; selected Player cards expose the shared centred clear icon through the existing undo path. | [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
