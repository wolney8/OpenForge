...
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `CODE-VERIFIED — USER RECHECK PENDING`: the banner is an explicit responsive grid: Hand/Current Hand/action icon and compact outcome buttons occupy the primary column; Last Action, waiting and completion status are separate rows in the secondary column. `Record outcome` is removed. Undo remains absolute and its shared icon box is centred within 1px. The top controls use three semantic grid rows (Deal/Reset, count/Surrender, Soft 17) with no spacer component and deliberate container-width reflow. Existing dual-pane, session-history, Last Hand, outcome, Bust and dealer-up-card-only guarantees remain unchanged. | [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
