...
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `PROVEN LOCALLY — WILL ACCEPTANCE PENDING`: Last Hand uses the animated full-width disclosure for click/automatic transitions without a side track, ghost width or React `inert` warning; rendered checks hold both Dealer/Player panel dimensions invariant. UP-CARD, every selected CARD N and banner undo share one centred target/glyph primitive without moving labels. Money-mode history visibly exposes aligned committed STAKED, RETURNED and NET P&L and remains contained at half/narrow widths. Settled Live/Free cards remain current until Player-header Rebet & Deal Again or Double & Deal Again starts the next hand. | [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
