...
| `PD-FUTURE-019` | Feature / payout-derived reference odds inside Profit Boost | `COMPLETE locally` at `b92728b9`; inputs and derivation history remain temporary; user/hosted acceptance pending | Profit Boost contracts; [#83](https://github.com/wolney8/OpenForge/issues/83) |
...
| `UI-COPY-001`, `UI-CONSISTENCY-001` | Visual / remove redundant “Tracker platform” subtitle and verify composed layout across existing/future surfaces in bounded batches | `PARTIAL`: Notification History toolbar checked; remaining search/filter/action toolbars, then repeated form/header layouts, remain visibly unchecked in the audit backlog; no broad redesign or app-wide compliance claim | [UI audit backlog](../agent-contracts/plum-duff-ui-audit-backlog.md); [#92](https://github.com/wolney8/OpenForge/issues/92) |
| `FINANCIAL-MOTION-001` | Presentation / true shared digit odometer for read-only signed money | `IN PROGRESS`: shared `FinancialValue` preserves static geometry and canonical clipboard text. The nearest semantic card/row group coordinates all contained values on one pointer entry, every explicit container click and leave/re-entry; nested groups remain isolated. Sign/width-stable zero-digit origins, neutral static `£ -`, persisted On/Off and 1.5-second/520ms/80ms timing defaults remain. Local focused automation passed; user/hosted acceptance and prospective new-surface adoption remain open | [Presentation contract](../contracts/financial-value-presentation-contract.md); [fixture spec](../fixture-specs/financial-value-presentation-fixture-spec.md); [#105](https://github.com/wolney8/OpenForge/issues/105) |
| `FINANCIAL-CHART-MOTION-001` | Presentation / pie, donut and progress-chart sweep/reveal under the shared motion preference | `IN PROGRESS`: first shared progress primitives cover Dashboard Target Progress, grouped Module Mix bars and the Operational Focus ring. They reveal to exact final state, coordinate with card FinancialValues, settle without idle effects, and obey persisted motion Off/reduced motion. Other chart families remain rollout scope; local rendered automation passed and user/hosted acceptance is pending | [Presentation contract](../contracts/financial-value-presentation-contract.md); [fixture spec](../fixture-specs/financial-value-presentation-fixture-spec.md); [#110](https://github.com/wolney8/OpenForge/issues/110) |
...
| `ACCOUNT-OPTION-RAIL-001` | UI / bounded Account quick-select rail with full authoritative select retained | `PARTIAL locally`: Extra Places bookmaker/win/place exchange rails rank the saved selection, latest same-workflow use, latest other persisted activity, positive balances and then canonical ties. All Extra Places quick-select groups show at most three choices with keyboard paging only when needed; authoritative selects retain every eligible Account. Other Account-backed consumers remain rollout scope | [#108](https://github.com/wolney8/OpenForge/issues/108) |
