...
|---|---|---|---|
| `CALCULATOR-BRIDGE-001` | Feature / immutable calculator or completed-session source into reviewed Profile ledger activity | `CODE-VERIFIED — USER RECHECK PENDING`: one product-neutral bridge now proves Standard → one/multiple Profile Sportsbook Prospecting rows and completed Blackjack Free/Live → exactly one reviewed Casino activity. Destination validation, Account blocks/warnings, per-target retry/idempotency, source checksum, durable notification and source-state retention are implemented. Simulation is ineligible; Blackjack promotion identity is mandatory and own cash maps to `Manual Play / No Offer`. Remaining eligible calculator-family adapters remain open under #36. `GITHUB RESULT COMMENT SYNC PENDING`: no authenticated connector/API token or `gh` executable was available after push. | Calculator bridge workflow; [#36](https://github.com/wolney8/OpenForge/issues/36), reused multi-Profile workflow [#77](https://github.com/wolney8/OpenForge/issues/77), Blackjack [#40](https://github.com/wolney8/OpenForge/issues/40) |
