...
|---|---|---|---|
| `CALCULATOR-WORKSPACE-001` | Feature / Fund Manager standalone calculator workspace | `PARTIAL locally; user acceptance PENDING RECHECK`: Standard, Multi-Lay, Extra Place / Each Way, Sequential Lay, Early Payout / 2UP and Odds / Probability are implemented reference-only families. Current copyable stakes share one centred copy/check icon target. Multiples, standalone Dutching and Blackjack remain blocked on the family-specific contracts below; #36 is later. | Calculator workspace workflow/fixtures; [#35](https://github.com/wolney8/OpenForge/issues/35), contracts/fixtures [#37](https://github.com/wolney8/OpenForge/issues/37), advanced catalogue [#38](https://github.com/wolney8/OpenForge/issues/38), Sequential Lay [#39](https://github.com/wolney8/OpenForge/issues/39), later bridge [#36](https://github.com/wolney8/OpenForge/issues/36) |
...
| `CALCULATOR-MULTIPLES-001` | Feature / standalone Multiples and Accumulator reference calculator | `BLOCKED`: the current accumulator captures are explicitly non-reactive and repeat one baseline after changed inputs. No approved family contract defines selection-state, stake-allocation, internal rounding, each-way, Rule 4 or all-winner bonus arithmetic. | M14 external-reference governance; [#38](https://github.com/wolney8/OpenForge/issues/38) |
| `CALCULATOR-DUTCHING-001` | Feature / standalone 2-way/3-way Normal and Free Bet Dutching | `BLOCKED`: the bridge fixture records no calculator contract. The approved two-way formula is scoped to the triggered Early Payout / 2UP workflow and cannot safely define the general family. | Calculator bridge fixtures; [#35](https://github.com/wolney8/OpenForge/issues/35), [#38](https://github.com/wolney8/OpenForge/issues/38) |
...
| `CALCULATOR-BLACKJACK-001` | Decision support / standalone Blackjack basic-strategy reference | `BLOCKED`: no approved game-rule strategy matrix or deterministic fixtures currently define soft/hard/split, surrender and dealer Soft-17 actions. | [#40](https://github.com/wolney8/OpenForge/issues/40) |
