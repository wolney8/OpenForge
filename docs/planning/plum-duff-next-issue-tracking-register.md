...
| `CALCULATOR-BLACKJACK-001` | Decision support / Simulation, Free Play and Live Play Blackjack sessions | `CODE-VERIFIED — USER RECHECK PENDING`: session and strategy behaviour is retained. Completed Free/Live history can now be reviewed and saved through the shared #36 bridge to exactly one eligible Profile/Casino Account while preserving the immutable `blackjack-session-v1` checksum. Simulation remains ineligible; promotion identity is required and own cash maps truthfully to `Manual Play / No Offer`. Wider #40 acceptance remains open. | `blackjack-session-v1`, `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), Casino semantics [#78](https://github.com/wolney8/OpenForge/issues/78), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `CODE-VERIFIED — USER RECHECK PENDING`: the explicit banner/top-control grids remain intact. Convertible-session money fields remain above the Dealer/Player table. Activity Source is explicit and independent; optional Digital/RNG versus Live Dealer provenance now sits in the shared Session details disclosure. Blocked mode changes leave mode unchanged, render the exact sentence-case error beneath Session Mode, dismiss on focus-away/four seconds and expose the existing confirmed Reset Session flow. Shared money fields normalise leading decimals on commit and retain malformed text for validation. Existing strategy/session result contracts are unchanged. | [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
