...
| `CALCULATOR-BLACKJACK-001` | Decision support / interactive standalone Blackjack basic-strategy session | `CODE-VERIFIED — USER RECHECK PENDING`: proven matrices/rank values are unchanged. Stake accounting is removed; explicit session-only outcomes do not grade strategy correctness; picker/dealer/player/empty states share one original vector-card primitive; Deal Again and anchored shared help use canonical controls. DAS remains the published matrix assumption; resplitting and casino P&L simulation remain uninferred. | `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
