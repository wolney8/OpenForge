...
| `CALCULATOR-BLACKJACK-001` | Decision support / interactive standalone Blackjack basic-strategy session | `PROVEN locally — CORE; USER/HOSTED ACCEPTANCE PENDING`: representative current Outplayed hard/soft/pair, H17/S17, surrender/fallback and hit-follow-up cases match the public calculator/matrices. The calculator records legal Hit/Stand/Double/Split/Surrender choices, exact temporary stake commitments, one split into independently played hands, and expandable browser-tab session history without Profile/ledger writes. DAS is assumed from the published conditional matrix; resplitting and outcome/P&L simulation are intentionally not inferred. | `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40) |
