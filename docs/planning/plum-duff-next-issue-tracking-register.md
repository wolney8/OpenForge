...
| `CALCULATOR-BLACKJACK-001` | Decision support / Simulation, Free Play and Live Play Blackjack sessions | `CODE-VERIFIED — USER RECHECK PENDING`: Simulation stays non-financial/non-convertible; Free Play separates free credit from withdrawable value; Live Play records actual per-hand stakes/returns and derives the reviewed session result only from ending minus starting balance. Actual Double/Split changes commitment, recommendations do not. Session details remain authenticated-session-local and the deterministic checksum snapshot performs no business write. The prominent recommendation/legal-action strip remains above the Dealer/Player row; the later #36 bridge remains pending. | `blackjack-session-v1`, `standalone-calculator-families-v1`; [#40](https://github.com/wolney8/OpenForge/issues/40), bridge [#36](https://github.com/wolney8/OpenForge/issues/36), Casino semantics [#78](https://github.com/wolney8/OpenForge/issues/78), consistency [#92](https://github.com/wolney8/OpenForge/issues/92) |
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `CODE-VERIFIED — USER RECHECK PENDING`: one top-right icon Undo reverses card/action/outcome transitions without moving the structured banner rows. Container-responsive Dealer/Player/cards pass dual-pane and narrow containment. Session History is an open-by-default, session-persisted disclosure. The aligned shared-card Last Hand recap fades after the next initial three cards; the redundant Restore action is removed. Semantic outcome actions expose `Blackjack Win` only for an original two-card natural, while Push remains available for a dealer tie. Card ranks use Times serif without changing app typography. Bust remains automatic/reduced-motion-safe, and only the dealer up-card is stored/sent. Strategy and `blackjack-session-v1` remain compatible. | [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
