...
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `CODE-VERIFIED — USER RECHECK PENDING`: the full-width action strip retains aligned completion controls and now exposes last action/waiting state plus a single snapshot-based Undo for card/action/Split transitions. Outcome archival remains duplicate-safe, starts a blank hand, and feeds a compact shared-card Last Hand recap with pristine-only restore. Bust is objective, automatic and briefly visible with reduced-motion protection. Only the dealer up-card is stored/sent; later dealer cards are excluded. Blocked Session mode changes retain inline validation. Focused desktop/narrow/text-scale/theme interaction checks pass; strategy and `blackjack-session-v1` remain compatible. | [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
