...
| `PD-FUTURE-001`–`PD-FUTURE-018` | Coverage recovery / preserve original future requests | `COVERAGE GAP`: no original requirement text was found in tracked files or Git history; do not reconstruct it from numbering | Original source unavailable; `SYNC PENDING` until requirements are recovered |
...
| `UI-COPY-001`, `UI-CONSISTENCY-001` | Visual / remove redundant “Tracker platform” subtitle, then review drift in bounded batches | `NOT STARTED`; no broad redesign | UI audit backlog; [#92](https://github.com/wolney8/OpenForge/issues/92) |
...
| `REBRANDING-DECISION-001` | Product decision / defer any further rename until a name is selected | `DEFERRED`; preserve compatibility identifiers | Current tracking request; historical implementation [#65](https://github.com/wolney8/OpenForge/issues/65); future issue `SYNC PENDING` until a name is selected |
...
| Product rebranding | Future product decision | Existing compatibility identifiers remain authoritative | Migration fixtures required after a name is selected | Deferred until the name is selected. No mass renaming of paths, schemas, storage keys, selectors, APIs, contracts or export identifiers. |
