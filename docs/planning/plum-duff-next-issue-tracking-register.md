...
| `FINANCIAL-MOTION-001` | Presentation / true shared digit odometer for read-only signed money | `IN PROGRESS`: shared `FinancialValue` preserves static geometry and canonical clipboard text, includes neutral-zero and deterministic hover/click/group replay, and is governed by the persisted default-On Fund Manager motion preference with reduced-motion override. Current correction adds a 2.5-second replay cooldown, a presentation-only `£ 0.00` origin for every signed destination, and shared row replay from financial or non-financial cells across active ledger/report/analytics/operational tables containing money. Future surfaces remain prospective rollout; automated and user/hosted verification are pending | [Presentation contract](../contracts/financial-value-presentation-contract.md); [fixture spec](../fixture-specs/financial-value-presentation-fixture-spec.md); [#105](https://github.com/wolney8/OpenForge/issues/105) |
| `FINANCIAL-CHART-MOTION-001` | Presentation / pie, donut and progress-chart sweep/reveal under the shared motion preference | `NOT STARTED`: load and hover/click replay, reduced-motion override and unchanged final chart data remain a separate tranche and do not block calculator work | [#110](https://github.com/wolney8/OpenForge/issues/110) |
