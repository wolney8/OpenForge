...
| `PD-FIX-BLACKJACK-COMPOSITION-001` | UI correction / compact Blackjack live-use composition | `CODE-VERIFIED — USER RECHECK PENDING`: the explicit banner/top-control grids remain intact. All conditional session/hand money inputs now occupy compact semantic rows above the Dealer/Player table, so Simulation/Free/Live changes cannot disturb card alignment. Table Type is the shared M3 `Live Dealer` switch over existing `digital_rng`/`live_dealer` values. A blocked mode change leaves mode unchanged, renders its associated sentence-case error beneath Session Mode and dismisses after four seconds. Shared BlackjackCard hover/focus holds one tilt direction per interaction; selection performs one bounded flip, guards duplicate callbacks and is static with reduced motion. Existing strategy/session contracts are unchanged. | [#40](https://github.com/wolney8/OpenForge/issues/40), consistency [#92](https://github.com/wolney8/OpenForge/issues/92), workspace [#35](https://github.com/wolney8/OpenForge/issues/35) |
