...
| PD-FR-007H61 | Deterministic legacy text | Shorten only constrained canonical text while retaining the full source value and normalization rule in persisted import provenance | Existing ledger field limits and import write plan | COMPLETE |
| PD-FR-007H62 | Historical offer labels | Generate neutral deterministic Casino labels only where the target requires a name and mark the value migration-generated | Casino import field contract | COMPLETE |
| PD-FR-007H63 | Lifecycle-safe missing strategy | Avoid review where strategy is semantically unnecessary for void/cancelled/non-executed or source-authoritative historical state | Workbook workflow and current-value contracts | COMPLETE |
| PD-FR-007H64 | Known advanced-lay mapping | Build canonical branch payloads for complete Multilay/Multilay-Underlay workbook rows and retain review for conflicting or incomplete branches | Sportsbook multi-lay contract | COMPLETE |
| PD-FR-007H65 | Genuine Sportsbook partials | Remove generic Sportsbook review when target, lifecycle, current value and source trace remain representable | Sportsbook import contract | COMPLETE |
| PD-FR-007H66 | Mapping v5 regression | Re-run the retained private checksum oracle with all rows accounted and zero week/month/year difference | Private developer dry-run evidence | COMPLETE |
| PD-FR-007H67 | Server-ready import action | Show `Import to Profile` only after the persisted server summary confirms the approved run has no blockers | Existing final summary and readiness gate | COMPLETE |
| PD-FR-007H68 | Final import confirmation | Retain the canonical high-consequence confirmation with target, checksum, Account/ledger impact and checkpoint availability | Existing confirmation dialog | COMPLETE |
| PD-FR-007H69 | Post-import reconciliation | Retain Neon re-read comparison for Accounts, ledgers, financial views, open positions, decisions and deterministic integrity | Existing cutover reconciliation service | COMPLETE |
| PD-FR-007H70 | Exportable reconciliation handoff | Persist and expose a concise structured reconciliation handoff for independent review after execution | Existing Import History reconciliation report | COMPLETE |
| PD-FR-007H71 | Hosted mapping v5 verification | Re-upload the current 719-row workbook, confirm reduced genuine review only, then verify controlled import on the Fund Manager-selected Profile | Vercel + Neon Profile import workflow | NEEDS VERIFICATION |
| PD-FR-007H72 | PostgreSQL import type parity | Convert integer-backed legacy boolean fields at the persistence boundary so approved Account and Cash Adjustment writes satisfy PostgreSQL | Existing repository storage adapter and Neon schema | COMPLETE |
| PD-FR-007H73 | Failed-import rollback proof | Verify zero committed write-audit rows and a Profile checksum equal to the pre-import checkpoint before allowing retry | Existing import transaction and checkpoint contract | COMPLETE |
| PD-FR-007H74 | Dry-run persistence preflight | Exercise the exact approved Profile, Account and ledger write set through canonical validators/constraints and force rollback before import | Existing cutover write service and transaction boundary | COMPLETE |
| PD-FR-007H75 | Approved-run retry lifecycle | Preserve checksum, decisions, reconciliation and plans while recording each failed execution attempt against the same approved run | Existing ImportRun audit model | COMPLETE |
| PD-FR-007H76 | Structured safe import failure | Show an authenticated stage/category summary, ImportRun ID, audit detail and safe retry validation without exposing database internals | Existing canonical error panel and import review workspace | COMPLETE |
| PD-FR-007H77 | Hosted approved-run retry | Validate and retry the preserved approved run on Vercel, then confirm persisted reconciliation from the completed import | Vercel + Neon Profile import workflow | NEEDS VERIFICATION |
| PD-FR-007H78 | Persisted import-state reconstruction | Treat the current ImportRun status as authoritative after navigation and retain an older terminal execution only as attempt history | Existing ImportRun workspace and execution monitor | COMPLETE |
| PD-FR-007H79 | Navigation-safe import start | Keep the bounded import-start request alive across navigation before persisted staged execution takes over | Existing staged execution endpoint and shell monitor | COMPLETE |
| PD-FR-007H80 | Review metadata deletion | Delete non-imported run/review/decision/checkpoint/audit metadata transactionally without raw workbook bytes and report action-specific errors | Existing destructive confirmation and ImportRun persistence | COMPLETE |
| PD-FR-008A1 | Canonical ledger provider branding | Resolve normal ledger provider cells through the master Account Catalogue and shared full-name provider chip | AccountProviderIdentity and stable-ID-first catalogue lookup | COMPLETE |
| PD-FR-008A2 | Opportunity Queue geometry | Constrain queue columns, wrap opportunity evidence and preserve canonical chip/action geometry | Signed-off dense table and local overflow patterns | COMPLETE |
| PD-FR-008A3 | Account signup opportunity lifecycle | Persist signup-offer availability on the Profile Account while retaining Profile Quick Actions as reusable definitions and ledger rows as execution instances | Existing Account lifecycle and Profile Quick Actions | COMPLETE |
| PD-FR-008A4 | Opportunity reminders | Surface pending signup, weekly and near-expiry opportunities through existing persisted notification preferences and deep links | Fund Manager Notifications | COMPLETE |
| PD-FR-008A5 | Import Profile-name authority | Preserve the target Plum Duff Profile name by default and require an explicit pre-approval choice to apply a workbook username | Existing import write-plan strategy pattern | COMPLETE |
...
| PD-FR-008L6 | Combined reporting performance | Remove PostgreSQL connection serialization and parallelize the existing summary serializers without changing calculations | Existing tracker summary bundle | IMPLEMENTED — HOSTED VERIFICATION PENDING |
| PD-FR-008L7 | Empty Profile directory state | Render zero-result Active/Archived/All filters as settled canonical empty states independent of secondary reporting | Existing Profile directory and loading hierarchy | IMPLEMENTED — HOSTED VERIFICATION PENDING |
| PD-FR-008L8 | Post-delete request lifecycle | Invalidate Profile caches, ignore stale deleted reads and prevent terminal 404 retry loops | Existing client cache and Profile deletion workflow | IMPLEMENTED — HOSTED VERIFICATION PENDING |
| PD-FR-008L9 | Fresh Profile Quick Actions | Make shipped Quick Action bootstrap atomic/idempotent and treat a successful zero-result list as the normal onboarding configuration state | Existing Fund Manager Quick Actions and onboarding loader | IMPLEMENTED — HOSTED VERIFICATION PENDING |
| PD-FR-008L10 | Optional onboarding import | Let Profile creation choose Start fresh or hand the created target Profile to the existing Import/Export dry-run workflow without duplicate setup entry | Existing Profile onboarding and workbook migration route | IMPLEMENTED — HOSTED VERIFICATION PENDING |
| PD-FR-008L11 | Permanent deletion audit | Retain a minimal immutable deletion tombstone with actor, timestamp and import identity outside the Profile data cascade | Existing transactional Profile deletion lifecycle | IMPLEMENTED — HOSTED VERIFICATION PENDING |
