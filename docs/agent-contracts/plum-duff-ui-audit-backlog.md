...
| P1 | Route guards, dirty-state prompts and destructive confirmations | Some flows still use browser/system `confirm` dialogs, which breaks visual consistency and cannot carry Plum Duff context, M3 styling or clear recovery copy | Replace with a shared in-app confirmation dialog service before broad route-guard expansion; migrate ledgers first | M3, WCAG, process logic |
